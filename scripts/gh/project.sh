...
# owner must first create a GitHub Project (v2) titled "FerroBRIDGE Roadmap" under
...
# fails loud with "no project titled 'FerroBRIDGE Roadmap'".
#
...
#
# The project is found by title (FERROBRIDGE_PROJECT_TITLE overrides; default
# "FerroBRIDGE Roadmap") under the repository owner.

set -euo pipefail

TITLE="${FERROBRIDGE_PROJECT_TITLE:-FerroBRIDGE Roadmap}"
...
# Post a project status update (createProjectV2StatusUpdate). The $start/$target
# variables are nullable Dates: omitted flags are simply not sent, which GraphQL
# reads as null.
cmd_update() {
...
# markers, never item bars), so this field exists solely as a machine-derived
# mirror: never hand-edit it, re-run this after changing a milestone due date
# or re-milestoning issues.
cmd_sync_dates() {
  resolve_project
  local field_id
