...
|---------------|--------------------------------------------------|-------------------|------------|
| Ruben Talstra | [@rubentalstra](https://github.com/rubentalstra) | Maintainer (sole) | 2026-09-03 |
...
inventory nobody can hand over. The project has no release yet, so the release
rows describe what the pipeline will use rather than what it has used.
...
| `GITHUB_TOKEN` (ephemeral, per workflow run) | the release artifacts, their SBOMs, and the Sigstore provenance | GitHub, minted per run; nothing stored | not applicable: there is no credential to lose                                                                                                                                              |
...
- **Nothing already published disappears.** A published release is immutable
  and its tag cannot be moved or deleted, so its assets stay downloadable; a
  deployment already running is not affected by maintainer availability.
...
- **The work is not lost.** The licence is the Business Source License 1.1,
  ...
  tree or on the tracker. A fork is a complete and legitimate continuation, and the
...
- **A vulnerability report has a fallback.** [SECURITY.md](SECURITY.md) states
  ...
  situation is the ordinary one for unmaintained software, and public disclosure
  to protect other users becomes your call. That path does not depend on the
  maintainer.
