...
the `.claude/` rules, agents, skills, and memory, doc comments, and every
commit, PR, and issue body. It does not touch vendored specifications. It does
not loosen the technical rules; citations, honesty, and the comment budgets in
`comments.md` still apply.
...
1. **The "not X, but Y" setup.** Do not frame a point as a contrast for effect
   ...
   "never simply"). State what the thing is. A contrast is allowed only when
   the reader genuinely holds the wrong belief and the sentence corrects it
   with facts on both sides.
...
6. **Adverb tics and hedging.** Cut "quietly", "genuinely", "simply",
   "notably", "importantly", "seamlessly" when they add nothing. State the
   fact.
7. **The TED-talk tone.** No inspirational build-ups, no rhetorical questions
   for effect, no "imagine a world where".
...
v1", "deferred", or "out of scope for now". The tracker is the scope, and
...
Prose has no lint, so this is review-enforced: hold every new doc, rule, PR,
and issue to it. Code comments are bounded by
