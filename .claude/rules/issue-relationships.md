...
The tracker is GitHub Issues (`issue-workflow.md`). GitHub exposes
...
trackable, individually closeable work items: for example "vendor and validate
the FHIRconnect schemas" with one child per schema, or "survey the prior art"
with one child per implementation read. Each child is a real issue with its own
...
(for example "the mapping loader" blocks "the mapping interpreter").
`scripts/gh/rel.sh blocked-by B A`.

An external wait with no in-repo counterpart (waiting on an upstream
specification release) stays a label, not a `blocked_by` edge; you cannot
