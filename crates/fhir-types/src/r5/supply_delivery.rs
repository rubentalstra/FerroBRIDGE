...
impl super::super::codec::Json for SupplyDelivery {
    fn to_json(&self) -> Result<super::super::codec::Object, super::super::codec::EncodeError> {
        ...
        if !self.supplied_item.is_empty() {
            let mut items = Vec::with_capacity(self.supplied_item.len());
            for item in &self.supplied_item {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
    ...
    fn from_json(
        ...
        let field_type = raw_type
            ...
            .transpose()?;
        let mut field_supplied_item = Vec::new();
        if let Some(raw) = raw_supplied_item {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_supplied_item.push(path.with_index("suppliedItem", index, |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(value, path)?,
                        path,
                    )
