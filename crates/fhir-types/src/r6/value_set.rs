...
pub struct ValueSetComposeIncludeFilter {
    ...
    /// child-of | descendent-leaf | exists | property-value-of
    ///
    ...
    /// specifies the set of concept codes that are the \`value\` the operator acts
    /// on.
    pub filter: Vec<ValueSetComposeIncludeFilter>,
}
...
pub struct ValueSetExpansion {
    ...
    /// return more using the offset parameter. The \`total\` element SHOULD be
    /// present in an expansion. If the \`total\` element is not present, the
    /// expansion SHALL be treated as one page in a paged expansion.
