...
pub struct CodeSystemValidateCodeRequest {
    ...
    /// codes are not considered to be valid. Note that 'abstract' is a property
    /// defined by many HL7 code systems that indicates that the concept is a
    /// logical grouping concept that is not intended to be used asa 'concrete'
    /// concept to in an actual patient/care/process record. This language is
    /// borrowed from Object Orienated theory where 'abstract' objects are never
    /// instantiated. However in the general record and terminology eco-system,
    /// there are many contexts where it is appropraite to use these codes e.g.
