// @generated by fhir-codegen from hl7.fhir.r6.core 6.0.0-ballot5. DO NOT EDIT.
// Change the emitter (tools/fhir-codegen) and regenerate.
// SPDX-FileCopyrightText: Vernum Projecten B.V.
// SPDX-License-Identifier: Apache-2.0
//! Results of a measure evaluation

/// Results of a measure evaluation
///
/// The MeasureReport resource contains the results of the calculation of a
/// measure; and optionally a reference to the resources involved in that
/// calculation.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct MeasureReport {
    /// Logical id of this artifact
    ///
    /// The logical id of the resource, as used in the URL for the resource. Once
    /// assigned, this value never changes.
    pub id: Option<std::string::String>,
    /// Metadata about the resource
    ///
    /// The metadata about the resource. This is content that is maintained by the
    /// infrastructure. Changes to the content might not always be associated with
    /// version changes to the resource.
    pub meta: Option<super::meta::Meta>,
    /// A set of rules under which this content was created
    ///
    /// A reference to a set of rules that were followed when the resource was
    /// constructed, and which must be understood when processing the content.
    /// Often, this is a reference to an implementation guide that defines the
    /// special rules along with other profiles etc. For DomainResources, if more
    /// than one implicitRules policy applies, additional policies may be conveyed
    /// using the
    /// \[additionalImplicitRules\](<https://build.fhir.org/ig/HL7/fhir-extensions/StructureDefinition-additionalImplicitRules.html>)
    /// extension.
    pub implicit_rules: Option<super::primitives::Uri>,
    /// Language of the resource content
    ///
    /// The base language in which the resource is written.
    pub language: Option<super::primitives::Code>,
    /// Text summary of the resource, for human interpretation
    ///
    /// A human-readable narrative that contains a summary of the resource and can
    /// be used to represent the content of the resource to a human. The narrative
    /// need not encode all the structured data, but is required to contain
    /// sufficient detail to make it "clinically safe" for a human to just read the
    /// narrative. Resource definitions may define what content should be
    /// represented in the narrative to ensure clinical safety.
    pub text: Option<super::narrative::Narrative>,
    /// Contained, inline Resources
    ///
    /// These resources do not have an independent existence apart from the resource
    /// that contains them - they cannot be identified independently, nor can they
    /// have their own independent transaction scope. This is allowed to be a
    /// Parameters resource if and only if it is referenced by a resource that
    /// provides context/meaning.
    pub contained: Vec<super::resource::Resource>,
    /// Additional content defined by implementations
    ///
    /// May be used to represent additional information that is not part of the
    /// basic definition of the resource. To make the use of extensions safe and
    /// managable, there is a strict set of governance applied to the definition and
    /// use of extensions. Though any implementer can define an extension, there is
    /// a set of requirements that SHALL be met as part of the definition of the
    /// extension.
    pub extension: Vec<super::extension::Extension>,
    /// Extensions that cannot be ignored
    ///
    /// May be used to represent additional information that is not part of the
    /// basic definition of the resource and that modifies the understanding of the
    /// element that contains it and/or the understanding of the containing
    /// element's descendants. Usually modifier elements provide negation or
    /// qualification. To make the use of extensions safe and managable, there is a
    /// strict set of governance applied to the definition and use of extensions.
    /// Though any implementer is allowed to define an extension, there is a set of
    /// requirements that SHALL be met as part of the definition of the extension.
    /// Applications processing a resource are required to check for modifier
    /// extensions. Modifier extensions SHALL NOT change the meaning of any elements
    /// on Resource or DomainResource (including cannot change the meaning of
    /// modifierExtension itself).
    pub modifier_extension: Vec<super::extension::Extension>,
    /// Additional identifier for the MeasureReport
    ///
    /// A formal identifier that is used to identify this MeasureReport when it is
    /// represented in other formats or referenced in a specification, model, design
    /// or an instance.
    pub identifier: Vec<super::identifier::Identifier>,
    /// The category of measure report instance this is (example codes include deqm,
    /// ra, vbp)
    ///
    /// The category of measure report instance this is such as Data Exchange for
    /// Quality Measures (DEQM), Risk Adjustment, or Value-Based Performance.
    pub category: Option<super::codeable_concept::CodeableConcept>,
    /// Evaluation messages
    ///
    /// A reference to an OperationOutcome that contains any information, warning,
    /// and/or error messages that were generated while processing an operation such
    /// as $evaluate.
    pub messages: Option<super::reference::Reference>,
    /// complete | pending | error
    ///
    /// The MeasureReport status. No data will be available until the MeasureReport
    /// status is complete. Pending means the report is in progress and in the
    /// future should lead to a complete report or an error report. Systems that do
    /// not know the status of the measure report should use a status of "error" and
    /// a message that explains that the status is unknown.
    pub status: super::primitives::Code,
    /// individual | subject-list | summary | data-exchange | other
    ///
    /// The type of measure report. This may be an individual report, which provides
    /// the score for the measure for an individual member of the population; a
    /// subject-listing, which returns the list of members that meet the various
    /// criteria in the measure; a summary report, which returns a population count
    /// for each of the criteria in the measure; or a data-collection, which enables
    /// the MeasureReport to be used to exchange the data-of-interest for a quality
    /// measure. The code "other" should be used if the report type is known, but it
    /// is not one of the enumerated report types. When "other" is used, an
    /// extension SHOULD be added to provide information on the type of measure
    /// report. The code "other" SHOULD NOT be used if the report type is unknown or
    /// undefined.
    pub r#type: Option<super::primitives::Code>,
    /// incremental | snapshot
    ///
    /// Indicates whether the data submitted in a data-exchange report represents a
    /// snapshot or incremental update. A snapshot update replaces all previously
    /// submitted data for the receiver, whereas an incremental update represents
    /// only updated and/or changed data and should be applied as a differential
    /// update to the existing submitted data for the receiver.
    pub data_update_type: Option<super::primitives::Code>,
    /// What measure was calculated
    ///
    /// A reference to the Measure that was calculated to produce this report.
    pub measure: Option<super::primitives::Canonical>,
    /// What individual(s) the report is for
    ///
    /// Optional subject identifying the individual or individuals the report is
    /// for.
    pub subject: Option<super::reference::Reference>,
    /// When the measure report was generated
    ///
    /// The date this measure report was generated.
    pub date: Option<super::primitives::DateTime>,
    /// Who is reporting the data
    ///
    /// The individual or organization that is reporting the data.
    pub reporter: Option<super::reference::Reference>,
    /// What vendor prepared the data
    ///
    /// A reference to the vendor who queried the data, calculated results and/or
    /// generated the report. The ‘reporting vendor’ is intended to represent the
    /// submitting entity when it is not the same as the reporting entity. This
    /// extension is used when the Receiver is interested in getting vendor
    /// information in the report.
    pub reporting_vendor: Option<super::reference::Reference>,
    /// Where the reported data is from
    ///
    /// A reference to the location for which the data is being reported.
    pub location: Vec<super::reference::Reference>,
    /// What period the report covers
    ///
    /// The reporting period for which the report was calculated.
    pub period: super::period::Period,
    /// What parameters were provided to the report
    ///
    /// A reference to a Parameters resource (typically represented using a
    /// contained resource) that represents any input parameters that were provided
    /// to the operation that generated the report. If the input parameter includes
    /// stratifiers, then the MeasureReport SHALL include the input parameters.
    pub input_parameters: Option<super::reference::Reference>,
    /// Measure results for each group
    ///
    /// The results of the calculation, one for each population group in the
    /// measure. A MeasureReport SHALL have a group element corresponding to each
    /// group element defined in the Measure being reported.
    pub group: Vec<MeasureReportGroup>,
    /// Additional information collected for the report
    ///
    /// A reference to a Resource that represents additional information collected
    /// for the report. If the value of the supplemental data is not a Resource
    /// (i.e. evaluating the supplementalData expression for this case in the
    /// measure results in a value that is not a FHIR Resource), it is reported as a
    /// reference to a contained Observation resource.
    pub supplemental_data: Vec<super::reference::Reference>,
    /// What data was used to calculate the measure score
    ///
    /// Evaluated resources are used to capture what data was involved in the
    /// calculation of a measure. This usage is only allowed for individual reports
    /// to ensure that the size of the MeasureReport resource is bounded.
    pub evaluated_resource: Vec<super::reference::Reference>,
}

impl super::super::codec::Json for MeasureReport {
    fn to_json(&self) -> Result<super::super::codec::Object, super::super::codec::EncodeError> {
        let mut object = super::super::codec::Object::new();
        object.insert(
            std::string::String::from("resourceType"),
            super::super::codec::Value::String(std::string::String::from("MeasureReport")),
        );
        if let Some(v) = &self.id {
            object.insert(
                std::string::String::from("id"),
                super::super::codec::Value::String(v.clone()),
            );
        }
        if let Some(item) = &self.meta {
            object.insert(
                std::string::String::from("meta"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if let Some(item) = &self.implicit_rules {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("implicitRules"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_implicitRules"), e);
            }
        }
        if let Some(item) = &self.language {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("language"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_language"), e);
            }
        }
        if let Some(item) = &self.text {
            object.insert(
                std::string::String::from("text"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if !self.contained.is_empty() {
            let mut items = Vec::with_capacity(self.contained.len());
            for item in &self.contained {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("contained"),
                super::super::codec::Value::Array(items),
            );
        }
        if !self.extension.is_empty() {
            let mut items = Vec::with_capacity(self.extension.len());
            for item in &self.extension {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("extension"),
                super::super::codec::Value::Array(items),
            );
        }
        if !self.modifier_extension.is_empty() {
            let mut items = Vec::with_capacity(self.modifier_extension.len());
            for item in &self.modifier_extension {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("modifierExtension"),
                super::super::codec::Value::Array(items),
            );
        }
        if !self.identifier.is_empty() {
            let mut items = Vec::with_capacity(self.identifier.len());
            for item in &self.identifier {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("identifier"),
                super::super::codec::Value::Array(items),
            );
        }
        if let Some(item) = &self.category {
            object.insert(
                std::string::String::from("category"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if let Some(item) = &self.messages {
            object.insert(
                std::string::String::from("messages"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if let Some(v) = super::super::codec::Primitive::value_json(&self.status)? {
            object.insert(std::string::String::from("status"), v);
        }
        if let Some(e) = super::super::codec::Primitive::element_json(&self.status)? {
            object.insert(std::string::String::from("_status"), e);
        }
        if let Some(item) = &self.r#type {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("type"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_type"), e);
            }
        }
        if let Some(item) = &self.data_update_type {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("dataUpdateType"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_dataUpdateType"), e);
            }
        }
        if let Some(item) = &self.measure {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("measure"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_measure"), e);
            }
        }
        if let Some(item) = &self.subject {
            object.insert(
                std::string::String::from("subject"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if let Some(item) = &self.date {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("date"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_date"), e);
            }
        }
        if let Some(item) = &self.reporter {
            object.insert(
                std::string::String::from("reporter"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if let Some(item) = &self.reporting_vendor {
            object.insert(
                std::string::String::from("reportingVendor"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if !self.location.is_empty() {
            let mut items = Vec::with_capacity(self.location.len());
            for item in &self.location {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("location"),
                super::super::codec::Value::Array(items),
            );
        }
        object.insert(
            std::string::String::from("period"),
            super::super::codec::Value::Object(super::super::codec::Json::to_json(&self.period)?),
        );
        if let Some(item) = &self.input_parameters {
            object.insert(
                std::string::String::from("inputParameters"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if !self.group.is_empty() {
            let mut items = Vec::with_capacity(self.group.len());
            for item in &self.group {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("group"),
                super::super::codec::Value::Array(items),
            );
        }
        if !self.supplemental_data.is_empty() {
            let mut items = Vec::with_capacity(self.supplemental_data.len());
            for item in &self.supplemental_data {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("supplementalData"),
                super::super::codec::Value::Array(items),
            );
        }
        if !self.evaluated_resource.is_empty() {
            let mut items = Vec::with_capacity(self.evaluated_resource.len());
            for item in &self.evaluated_resource {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("evaluatedResource"),
                super::super::codec::Value::Array(items),
            );
        }
        Ok(object)
    }

    fn from_json(
        object: &super::super::codec::Object,
        path: &mut super::super::codec::Path,
    ) -> Result<Self, super::super::codec::DecodeError> {
        let mut raw_id: Option<&super::super::codec::Value> = None;
        let mut raw_meta: Option<&super::super::codec::Value> = None;
        let mut raw_implicit_rules: Option<&super::super::codec::Value> = None;
        let mut raw_implicit_rules_element: Option<&super::super::codec::Value> = None;
        let mut raw_language: Option<&super::super::codec::Value> = None;
        let mut raw_language_element: Option<&super::super::codec::Value> = None;
        let mut raw_text: Option<&super::super::codec::Value> = None;
        let mut raw_contained: Option<&super::super::codec::Value> = None;
        let mut raw_extension: Option<&super::super::codec::Value> = None;
        let mut raw_modifier_extension: Option<&super::super::codec::Value> = None;
        let mut raw_identifier: Option<&super::super::codec::Value> = None;
        let mut raw_category: Option<&super::super::codec::Value> = None;
        let mut raw_messages: Option<&super::super::codec::Value> = None;
        let mut raw_status: Option<&super::super::codec::Value> = None;
        let mut raw_status_element: Option<&super::super::codec::Value> = None;
        let mut raw_type: Option<&super::super::codec::Value> = None;
        let mut raw_type_element: Option<&super::super::codec::Value> = None;
        let mut raw_data_update_type: Option<&super::super::codec::Value> = None;
        let mut raw_data_update_type_element: Option<&super::super::codec::Value> = None;
        let mut raw_measure: Option<&super::super::codec::Value> = None;
        let mut raw_measure_element: Option<&super::super::codec::Value> = None;
        let mut raw_subject: Option<&super::super::codec::Value> = None;
        let mut raw_date: Option<&super::super::codec::Value> = None;
        let mut raw_date_element: Option<&super::super::codec::Value> = None;
        let mut raw_reporter: Option<&super::super::codec::Value> = None;
        let mut raw_reporting_vendor: Option<&super::super::codec::Value> = None;
        let mut raw_location: Option<&super::super::codec::Value> = None;
        let mut raw_period: Option<&super::super::codec::Value> = None;
        let mut raw_input_parameters: Option<&super::super::codec::Value> = None;
        let mut raw_group: Option<&super::super::codec::Value> = None;
        let mut raw_supplemental_data: Option<&super::super::codec::Value> = None;
        let mut raw_evaluated_resource: Option<&super::super::codec::Value> = None;
        for (key, value) in object {
            match key.as_str() {
                "resourceType" => {
                    if value.as_str() != Some("MeasureReport") {
                        return Err(path.error(super::super::codec::DecodeErrorKind::ResourceType));
                    }
                }
                "id" => {
                    raw_id = Some(value);
                }
                "meta" => {
                    raw_meta = Some(value);
                }
                "implicitRules" => {
                    raw_implicit_rules = Some(value);
                }
                "_implicitRules" => {
                    raw_implicit_rules_element = Some(value);
                }
                "language" => {
                    raw_language = Some(value);
                }
                "_language" => {
                    raw_language_element = Some(value);
                }
                "text" => {
                    raw_text = Some(value);
                }
                "contained" => {
                    raw_contained = Some(value);
                }
                "extension" => {
                    raw_extension = Some(value);
                }
                "modifierExtension" => {
                    raw_modifier_extension = Some(value);
                }
                "identifier" => {
                    raw_identifier = Some(value);
                }
                "category" => {
                    raw_category = Some(value);
                }
                "messages" => {
                    raw_messages = Some(value);
                }
                "status" => {
                    raw_status = Some(value);
                }
                "_status" => {
                    raw_status_element = Some(value);
                }
                "type" => {
                    raw_type = Some(value);
                }
                "_type" => {
                    raw_type_element = Some(value);
                }
                "dataUpdateType" => {
                    raw_data_update_type = Some(value);
                }
                "_dataUpdateType" => {
                    raw_data_update_type_element = Some(value);
                }
                "measure" => {
                    raw_measure = Some(value);
                }
                "_measure" => {
                    raw_measure_element = Some(value);
                }
                "subject" => {
                    raw_subject = Some(value);
                }
                "date" => {
                    raw_date = Some(value);
                }
                "_date" => {
                    raw_date_element = Some(value);
                }
                "reporter" => {
                    raw_reporter = Some(value);
                }
                "reportingVendor" => {
                    raw_reporting_vendor = Some(value);
                }
                "location" => {
                    raw_location = Some(value);
                }
                "period" => {
                    raw_period = Some(value);
                }
                "inputParameters" => {
                    raw_input_parameters = Some(value);
                }
                "group" => {
                    raw_group = Some(value);
                }
                "supplementalData" => {
                    raw_supplemental_data = Some(value);
                }
                "evaluatedResource" => {
                    raw_evaluated_resource = Some(value);
                }
                other => {
                    return path.with(other, |path| {
                        Err(path.error(super::super::codec::DecodeErrorKind::UnknownProperty))
                    });
                }
            }
        }
        let field_id = raw_id
            .map(|value| {
                path.with("id", |path| {
                    let value = super::super::codec::expect_single(value, path)?;
                    super::primitives::checked_string(
                        super::super::codec::expect_string(value, path)?,
                        path,
                    )
                })
            })
            .transpose()?;
        let field_meta = raw_meta
            .map(|value| {
                path.with("meta", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let field_implicit_rules = match (raw_implicit_rules, raw_implicit_rules_element) {
            (None, None) => None,
            (value, element) => Some(path.with("implicitRules", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_language = match (raw_language, raw_language_element) {
            (None, None) => None,
            (value, element) => Some(path.with("language", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_text = raw_text
            .map(|value| {
                path.with("text", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let mut field_contained = Vec::new();
        if let Some(raw) = raw_contained {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_contained.push(path.with_index("contained", index, |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(value, path)?,
                        path,
                    )
                })?);
            }
        }
        let mut field_extension = Vec::new();
        if let Some(raw) = raw_extension {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_extension.push(path.with_index("extension", index, |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(value, path)?,
                        path,
                    )
                })?);
            }
        }
        let mut field_modifier_extension = Vec::new();
        if let Some(raw) = raw_modifier_extension {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_modifier_extension.push(path.with_index(
                    "modifierExtension",
                    index,
                    |path| {
                        super::super::codec::Json::from_json(
                            super::super::codec::expect_object(value, path)?,
                            path,
                        )
                    },
                )?);
            }
        }
        let mut field_identifier = Vec::new();
        if let Some(raw) = raw_identifier {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_identifier.push(path.with_index("identifier", index, |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(value, path)?,
                        path,
                    )
                })?);
            }
        }
        let field_category = raw_category
            .map(|value| {
                path.with("category", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let field_messages = raw_messages
            .map(|value| {
                path.with("messages", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let field_status = path.with("status", |path| {
            super::super::codec::Primitive::from_json_parts(raw_status, raw_status_element, path)
        })?;
        let field_type = match (raw_type, raw_type_element) {
            (None, None) => None,
            (value, element) => Some(path.with("type", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_data_update_type = match (raw_data_update_type, raw_data_update_type_element) {
            (None, None) => None,
            (value, element) => Some(path.with("dataUpdateType", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_measure = match (raw_measure, raw_measure_element) {
            (None, None) => None,
            (value, element) => Some(path.with("measure", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_subject = raw_subject
            .map(|value| {
                path.with("subject", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let field_date = match (raw_date, raw_date_element) {
            (None, None) => None,
            (value, element) => Some(path.with("date", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_reporter = raw_reporter
            .map(|value| {
                path.with("reporter", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let field_reporting_vendor = raw_reporting_vendor
            .map(|value| {
                path.with("reportingVendor", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let mut field_location = Vec::new();
        if let Some(raw) = raw_location {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_location.push(path.with_index("location", index, |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(value, path)?,
                        path,
                    )
                })?);
            }
        }
        let field_period = path.with("period", |path| {
            let value = raw_period
                .ok_or_else(|| path.error(super::super::codec::DecodeErrorKind::MissingProperty))?;
            super::super::codec::Json::from_json(
                super::super::codec::expect_object(
                    super::super::codec::expect_single(value, path)?,
                    path,
                )?,
                path,
            )
        })?;
        let field_input_parameters = raw_input_parameters
            .map(|value| {
                path.with("inputParameters", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let mut field_group = Vec::new();
        if let Some(raw) = raw_group {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_group.push(path.with_index("group", index, |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(value, path)?,
                        path,
                    )
                })?);
            }
        }
        let mut field_supplemental_data = Vec::new();
        if let Some(raw) = raw_supplemental_data {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_supplemental_data.push(path.with_index(
                    "supplementalData",
                    index,
                    |path| {
                        super::super::codec::Json::from_json(
                            super::super::codec::expect_object(value, path)?,
                            path,
                        )
                    },
                )?);
            }
        }
        let mut field_evaluated_resource = Vec::new();
        if let Some(raw) = raw_evaluated_resource {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_evaluated_resource.push(path.with_index(
                    "evaluatedResource",
                    index,
                    |path| {
                        super::super::codec::Json::from_json(
                            super::super::codec::expect_object(value, path)?,
                            path,
                        )
                    },
                )?);
            }
        }
        Ok(Self {
            id: field_id,
            meta: field_meta,
            implicit_rules: field_implicit_rules,
            language: field_language,
            text: field_text,
            contained: field_contained,
            extension: field_extension,
            modifier_extension: field_modifier_extension,
            identifier: field_identifier,
            category: field_category,
            messages: field_messages,
            status: field_status,
            r#type: field_type,
            data_update_type: field_data_update_type,
            measure: field_measure,
            subject: field_subject,
            date: field_date,
            reporter: field_reporter,
            reporting_vendor: field_reporting_vendor,
            location: field_location,
            period: field_period,
            input_parameters: field_input_parameters,
            group: field_group,
            supplemental_data: field_supplemental_data,
            evaluated_resource: field_evaluated_resource,
        })
    }
}

impl serde::Serialize for MeasureReport {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        let mut map = serde::Serializer::serialize_map(serializer, None)?;
        if let Some(item) = &self.data_update_type {
            super::super::codec::element_entry(&mut map, "_dataUpdateType", item)?;
        }
        if let Some(item) = &self.date {
            super::super::codec::element_entry(&mut map, "_date", item)?;
        }
        if let Some(item) = &self.implicit_rules {
            super::super::codec::element_entry(&mut map, "_implicitRules", item)?;
        }
        if let Some(item) = &self.language {
            super::super::codec::element_entry(&mut map, "_language", item)?;
        }
        if let Some(item) = &self.measure {
            super::super::codec::element_entry(&mut map, "_measure", item)?;
        }
        super::super::codec::element_entry(&mut map, "_status", &self.status)?;
        if let Some(item) = &self.r#type {
            super::super::codec::element_entry(&mut map, "_type", item)?;
        }
        if let Some(item) = &self.category {
            serde::ser::SerializeMap::serialize_entry(&mut map, "category", item)?;
        }
        if !self.contained.is_empty() {
            serde::ser::SerializeMap::serialize_entry(&mut map, "contained", &self.contained)?;
        }
        if let Some(item) = &self.data_update_type {
            super::super::codec::value_entry(&mut map, "dataUpdateType", item)?;
        }
        if let Some(item) = &self.date {
            super::super::codec::value_entry(&mut map, "date", item)?;
        }
        if !self.evaluated_resource.is_empty() {
            serde::ser::SerializeMap::serialize_entry(
                &mut map,
                "evaluatedResource",
                &self.evaluated_resource,
            )?;
        }
        if !self.extension.is_empty() {
            serde::ser::SerializeMap::serialize_entry(&mut map, "extension", &self.extension)?;
        }
        if !self.group.is_empty() {
            serde::ser::SerializeMap::serialize_entry(&mut map, "group", &self.group)?;
        }
        if let Some(v) = &self.id {
            serde::ser::SerializeMap::serialize_entry(&mut map, "id", v)?;
        }
        if !self.identifier.is_empty() {
            serde::ser::SerializeMap::serialize_entry(&mut map, "identifier", &self.identifier)?;
        }
        if let Some(item) = &self.implicit_rules {
            super::super::codec::value_entry(&mut map, "implicitRules", item)?;
        }
        if let Some(item) = &self.input_parameters {
            serde::ser::SerializeMap::serialize_entry(&mut map, "inputParameters", item)?;
        }
        if let Some(item) = &self.language {
            super::super::codec::value_entry(&mut map, "language", item)?;
        }
        if !self.location.is_empty() {
            serde::ser::SerializeMap::serialize_entry(&mut map, "location", &self.location)?;
        }
        if let Some(item) = &self.measure {
            super::super::codec::value_entry(&mut map, "measure", item)?;
        }
        if let Some(item) = &self.messages {
            serde::ser::SerializeMap::serialize_entry(&mut map, "messages", item)?;
        }
        if let Some(item) = &self.meta {
            serde::ser::SerializeMap::serialize_entry(&mut map, "meta", item)?;
        }
        if !self.modifier_extension.is_empty() {
            serde::ser::SerializeMap::serialize_entry(
                &mut map,
                "modifierExtension",
                &self.modifier_extension,
            )?;
        }
        serde::ser::SerializeMap::serialize_entry(&mut map, "period", &self.period)?;
        if let Some(item) = &self.reporter {
            serde::ser::SerializeMap::serialize_entry(&mut map, "reporter", item)?;
        }
        if let Some(item) = &self.reporting_vendor {
            serde::ser::SerializeMap::serialize_entry(&mut map, "reportingVendor", item)?;
        }
        serde::ser::SerializeMap::serialize_entry(&mut map, "resourceType", "MeasureReport")?;
        super::super::codec::value_entry(&mut map, "status", &self.status)?;
        if let Some(item) = &self.subject {
            serde::ser::SerializeMap::serialize_entry(&mut map, "subject", item)?;
        }
        if !self.supplemental_data.is_empty() {
            serde::ser::SerializeMap::serialize_entry(
                &mut map,
                "supplementalData",
                &self.supplemental_data,
            )?;
        }
        if let Some(item) = &self.text {
            serde::ser::SerializeMap::serialize_entry(&mut map, "text", item)?;
        }
        if let Some(item) = &self.r#type {
            super::super::codec::value_entry(&mut map, "type", item)?;
        }
        serde::ser::SerializeMap::end(map)
    }
}

impl<'de> serde::Deserialize<'de> for MeasureReport {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let value = <super::super::codec::Value as serde::Deserialize>::deserialize(deserializer)?;
        let mut path = super::super::codec::Path::root("MeasureReport");
        let object =
            super::super::codec::expect_object(&value, &path).map_err(serde::de::Error::custom)?;
        super::super::codec::Json::from_json(object, &mut path).map_err(serde::de::Error::custom)
    }
}

/// Measure results for each group
///
/// The results of the calculation, one for each population group in the
/// measure. A MeasureReport SHALL have a group element corresponding to each
/// group element defined in the Measure being reported.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct MeasureReportGroup {
    /// Unique id for inter-element referencing
    ///
    /// Unique id for the element within a resource (for internal references). This
    /// may be any string value that does not contain spaces.
    pub id: Option<std::string::String>,
    /// Additional content defined by implementations
    ///
    /// May be used to represent additional information that is not part of the
    /// basic definition of the element. To make the use of extensions safe and
    /// managable, there is a strict set of governance applied to the definition and
    /// use of extensions. Though any implementer can define an extension, there is
    /// a set of requirements that SHALL be met as part of the definition of the
    /// extension.
    pub extension: Vec<super::extension::Extension>,
    /// Extensions that cannot be ignored even if unrecognized
    ///
    /// May be used to represent additional information that is not part of the
    /// basic definition of the element and that modifies the understanding of the
    /// element in which it is contained and/or the understanding of the containing
    /// element's descendants. Usually modifier elements provide negation or
    /// qualification. To make the use of extensions safe and managable, there is a
    /// strict set of governance applied to the definition and use of extensions.
    /// Though any implementer can define an extension, there is a set of
    /// requirements that SHALL be met as part of the definition of the extension.
    /// Applications processing a resource are required to check for modifier
    /// extensions. Modifier extensions SHALL NOT change the meaning of any elements
    /// on Resource or DomainResource (including cannot change the meaning of
    /// modifierExtension itself).
    pub modifier_extension: Vec<super::extension::Extension>,
    /// Pointer to specific group from Measure
    ///
    /// The group from the Measure that corresponds to this group in the
    /// MeasureReport resource. This element SHALL be populated based on the
    /// corresponding element in the Measure being reported.
    pub link_id: Option<super::primitives::String>,
    /// Title of a group. Note- this value is copied from this element in Measure
    ///
    /// Indicates the title for of the group.
    pub title: Option<super::primitives::String>,
    /// The date the Measure Report was calculated.
    pub calculated_date: Option<super::primitives::DateTime>,
    /// Meaning of the group
    ///
    /// The meaning of the population group as defined in the measure definition.
    /// This element SHALL be populated with at least the codings in the code of the
    /// corresponding group in the Measure being reported.
    pub code: Option<super::codeable_concept::CodeableConcept>,
    /// Summary description
    ///
    /// The human readable description of this population group. This element SHOULD
    /// be populated based on the description of the corresponding group in the
    /// Measure being reported.
    pub description: Option<super::primitives::Markdown>,
    /// process | outcome | structure | patient-reported-outcome
    ///
    /// Indicates whether the measure is used to examine a process, an outcome over
    /// time, a patient-reported outcome, or a structure measure such as
    /// utilization.
    pub r#type: Option<super::codeable_concept::CodeableConcept>,
    /// What individual(s) the report is for
    ///
    /// Optional subject identifying the individual or individuals the report is
    /// for.
    pub subject: Option<super::reference::Reference>,
    /// What scoring method (e.g. proportion, ratio, continuous-variable)
    ///
    /// Indicates how the calculation is performed for the measure, including
    /// proportion, ratio, continuous-variable, and cohort. The value set is
    /// extensible, allowing additional measure scoring types to be represented. It
    /// is expected to be the same as the scoring element on the referenced Measure.
    pub scoring: Option<super::codeable_concept::CodeableConcept>,
    /// opportunity | all-or-nothing | linear | weighted
    ///
    /// If this is a composite measure, the scoring method used to combine the
    /// component measures to determine the composite score.
    pub composite_scoring: Option<super::codeable_concept::CodeableConcept>,
    /// increase | decrease
    ///
    /// Whether improvement in the measure is noted by an increase or decrease in
    /// the measure score. Exercise caution when using any values besides increase
    /// or decrease for improvementNotation.
    pub improvement_notation: Option<super::codeable_concept::CodeableConcept>,
    /// Explanation of improvement notation
    ///
    /// Narrative text to explain the improvement notation and how to interpret it.
    pub improvement_notation_guidance: Option<super::primitives::Markdown>,
    /// The populations in the group
    ///
    /// The populations that make up the population group, one for each type of
    /// population appropriate for the measure. Each group in the MeasureReport
    /// SHALL have populations as defined in the corresponding group of the Measure
    /// being reported.
    pub population: Vec<MeasureReportGroupPopulation>,
    /// What score this group achieved
    ///
    /// The measure score for this population group, calculated as appropriate for
    /// the measure type and scoring method, and based on the contents of the
    /// populations defined in the group.
    pub measure_score: Option<MeasureReportGroupMeasureScore>,
    /// Stratification results
    ///
    /// The stratification results for this measure group, calculated as defined by
    /// the stratifier element of the measure being reported. Each group in the
    /// MeasureReport SHALL have stratifiers as defined in the corresponding group
    /// of the Measure being reported.
    pub stratifier: Vec<MeasureReportGroupStratifier>,
}

impl super::super::codec::Json for MeasureReportGroup {
    fn to_json(&self) -> Result<super::super::codec::Object, super::super::codec::EncodeError> {
        let mut object = super::super::codec::Object::new();
        if let Some(v) = &self.id {
            object.insert(
                std::string::String::from("id"),
                super::super::codec::Value::String(v.clone()),
            );
        }
        if !self.extension.is_empty() {
            let mut items = Vec::with_capacity(self.extension.len());
            for item in &self.extension {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("extension"),
                super::super::codec::Value::Array(items),
            );
        }
        if !self.modifier_extension.is_empty() {
            let mut items = Vec::with_capacity(self.modifier_extension.len());
            for item in &self.modifier_extension {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("modifierExtension"),
                super::super::codec::Value::Array(items),
            );
        }
        if let Some(item) = &self.link_id {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("linkId"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_linkId"), e);
            }
        }
        if let Some(item) = &self.title {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("title"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_title"), e);
            }
        }
        if let Some(item) = &self.calculated_date {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("calculatedDate"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_calculatedDate"), e);
            }
        }
        if let Some(item) = &self.code {
            object.insert(
                std::string::String::from("code"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if let Some(item) = &self.description {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("description"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_description"), e);
            }
        }
        if let Some(item) = &self.r#type {
            object.insert(
                std::string::String::from("type"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if let Some(item) = &self.subject {
            object.insert(
                std::string::String::from("subject"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if let Some(item) = &self.scoring {
            object.insert(
                std::string::String::from("scoring"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if let Some(item) = &self.composite_scoring {
            object.insert(
                std::string::String::from("compositeScoring"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if let Some(item) = &self.improvement_notation {
            object.insert(
                std::string::String::from("improvementNotation"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if let Some(item) = &self.improvement_notation_guidance {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("improvementNotationGuidance"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_improvementNotationGuidance"), e);
            }
        }
        if !self.population.is_empty() {
            let mut items = Vec::with_capacity(self.population.len());
            for item in &self.population {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("population"),
                super::super::codec::Value::Array(items),
            );
        }
        if let Some(item) = &self.measure_score {
            let (suffix, value, element) = item.to_json_parts()?;
            if let Some(value) = value {
                object.insert(format!("measureScore{suffix}"), value);
            }
            if let Some(element) = element {
                object.insert(format!("_measureScore{suffix}"), element);
            }
        }
        if !self.stratifier.is_empty() {
            let mut items = Vec::with_capacity(self.stratifier.len());
            for item in &self.stratifier {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("stratifier"),
                super::super::codec::Value::Array(items),
            );
        }
        Ok(object)
    }

    fn from_json(
        object: &super::super::codec::Object,
        path: &mut super::super::codec::Path,
    ) -> Result<Self, super::super::codec::DecodeError> {
        let mut raw_id: Option<&super::super::codec::Value> = None;
        let mut raw_extension: Option<&super::super::codec::Value> = None;
        let mut raw_modifier_extension: Option<&super::super::codec::Value> = None;
        let mut raw_link_id: Option<&super::super::codec::Value> = None;
        let mut raw_link_id_element: Option<&super::super::codec::Value> = None;
        let mut raw_title: Option<&super::super::codec::Value> = None;
        let mut raw_title_element: Option<&super::super::codec::Value> = None;
        let mut raw_calculated_date: Option<&super::super::codec::Value> = None;
        let mut raw_calculated_date_element: Option<&super::super::codec::Value> = None;
        let mut raw_code: Option<&super::super::codec::Value> = None;
        let mut raw_description: Option<&super::super::codec::Value> = None;
        let mut raw_description_element: Option<&super::super::codec::Value> = None;
        let mut raw_type: Option<&super::super::codec::Value> = None;
        let mut raw_subject: Option<&super::super::codec::Value> = None;
        let mut raw_scoring: Option<&super::super::codec::Value> = None;
        let mut raw_composite_scoring: Option<&super::super::codec::Value> = None;
        let mut raw_improvement_notation: Option<&super::super::codec::Value> = None;
        let mut raw_improvement_notation_guidance: Option<&super::super::codec::Value> = None;
        let mut raw_improvement_notation_guidance_element: Option<&super::super::codec::Value> =
            None;
        let mut raw_population: Option<&super::super::codec::Value> = None;
        let mut raw_measure_score = super::super::codec::ChoiceSlot::default();
        let mut raw_stratifier: Option<&super::super::codec::Value> = None;
        for (key, value) in object {
            match key.as_str() {
                "id" => {
                    raw_id = Some(value);
                }
                "extension" => {
                    raw_extension = Some(value);
                }
                "modifierExtension" => {
                    raw_modifier_extension = Some(value);
                }
                "linkId" => {
                    raw_link_id = Some(value);
                }
                "_linkId" => {
                    raw_link_id_element = Some(value);
                }
                "title" => {
                    raw_title = Some(value);
                }
                "_title" => {
                    raw_title_element = Some(value);
                }
                "calculatedDate" => {
                    raw_calculated_date = Some(value);
                }
                "_calculatedDate" => {
                    raw_calculated_date_element = Some(value);
                }
                "code" => {
                    raw_code = Some(value);
                }
                "description" => {
                    raw_description = Some(value);
                }
                "_description" => {
                    raw_description_element = Some(value);
                }
                "type" => {
                    raw_type = Some(value);
                }
                "subject" => {
                    raw_subject = Some(value);
                }
                "scoring" => {
                    raw_scoring = Some(value);
                }
                "compositeScoring" => {
                    raw_composite_scoring = Some(value);
                }
                "improvementNotation" => {
                    raw_improvement_notation = Some(value);
                }
                "improvementNotationGuidance" => {
                    raw_improvement_notation_guidance = Some(value);
                }
                "_improvementNotationGuidance" => {
                    raw_improvement_notation_guidance_element = Some(value);
                }
                "population" => {
                    raw_population = Some(value);
                }
                "measureScoreQuantity" => {
                    raw_measure_score.value("Quantity", value, path)?;
                }
                "_measureScoreQuantity" => {
                    raw_measure_score.element("Quantity", value, path)?;
                }
                "measureScoreDateTime" => {
                    raw_measure_score.value("DateTime", value, path)?;
                }
                "_measureScoreDateTime" => {
                    raw_measure_score.element("DateTime", value, path)?;
                }
                "measureScoreCodeableConcept" => {
                    raw_measure_score.value("CodeableConcept", value, path)?;
                }
                "_measureScoreCodeableConcept" => {
                    raw_measure_score.element("CodeableConcept", value, path)?;
                }
                "measureScorePeriod" => {
                    raw_measure_score.value("Period", value, path)?;
                }
                "_measureScorePeriod" => {
                    raw_measure_score.element("Period", value, path)?;
                }
                "measureScoreRange" => {
                    raw_measure_score.value("Range", value, path)?;
                }
                "_measureScoreRange" => {
                    raw_measure_score.element("Range", value, path)?;
                }
                "measureScoreDuration" => {
                    raw_measure_score.value("Duration", value, path)?;
                }
                "_measureScoreDuration" => {
                    raw_measure_score.element("Duration", value, path)?;
                }
                "measureScoreBoolean" => {
                    raw_measure_score.value("Boolean", value, path)?;
                }
                "_measureScoreBoolean" => {
                    raw_measure_score.element("Boolean", value, path)?;
                }
                "measureScoreRatio" => {
                    raw_measure_score.value("Ratio", value, path)?;
                }
                "_measureScoreRatio" => {
                    raw_measure_score.element("Ratio", value, path)?;
                }
                "stratifier" => {
                    raw_stratifier = Some(value);
                }
                other => {
                    return path.with(other, |path| {
                        Err(path.error(super::super::codec::DecodeErrorKind::UnknownProperty))
                    });
                }
            }
        }
        let field_id = raw_id
            .map(|value| {
                path.with("id", |path| {
                    let value = super::super::codec::expect_single(value, path)?;
                    super::primitives::checked_string(
                        super::super::codec::expect_string(value, path)?,
                        path,
                    )
                })
            })
            .transpose()?;
        let mut field_extension = Vec::new();
        if let Some(raw) = raw_extension {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_extension.push(path.with_index("extension", index, |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(value, path)?,
                        path,
                    )
                })?);
            }
        }
        let mut field_modifier_extension = Vec::new();
        if let Some(raw) = raw_modifier_extension {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_modifier_extension.push(path.with_index(
                    "modifierExtension",
                    index,
                    |path| {
                        super::super::codec::Json::from_json(
                            super::super::codec::expect_object(value, path)?,
                            path,
                        )
                    },
                )?);
            }
        }
        let field_link_id = match (raw_link_id, raw_link_id_element) {
            (None, None) => None,
            (value, element) => Some(path.with("linkId", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_title = match (raw_title, raw_title_element) {
            (None, None) => None,
            (value, element) => Some(path.with("title", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_calculated_date = match (raw_calculated_date, raw_calculated_date_element) {
            (None, None) => None,
            (value, element) => Some(path.with("calculatedDate", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_code = raw_code
            .map(|value| {
                path.with("code", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let field_description = match (raw_description, raw_description_element) {
            (None, None) => None,
            (value, element) => Some(path.with("description", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_type = raw_type
            .map(|value| {
                path.with("type", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let field_subject = raw_subject
            .map(|value| {
                path.with("subject", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let field_scoring = raw_scoring
            .map(|value| {
                path.with("scoring", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let field_composite_scoring = raw_composite_scoring
            .map(|value| {
                path.with("compositeScoring", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let field_improvement_notation = raw_improvement_notation
            .map(|value| {
                path.with("improvementNotation", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let field_improvement_notation_guidance = match (
            raw_improvement_notation_guidance,
            raw_improvement_notation_guidance_element,
        ) {
            (None, None) => None,
            (value, element) => Some(path.with("improvementNotationGuidance", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let mut field_population = Vec::new();
        if let Some(raw) = raw_population {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_population.push(path.with_index("population", index, |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(value, path)?,
                        path,
                    )
                })?);
            }
        }
        let field_measure_score = match raw_measure_score.suffix {
            None => None,
            Some(suffix) => Some(path.with("measureScore", |path| {
                MeasureReportGroupMeasureScore::from_json_parts(
                    suffix,
                    raw_measure_score.value,
                    raw_measure_score.element,
                    path,
                )
            })?),
        };
        let mut field_stratifier = Vec::new();
        if let Some(raw) = raw_stratifier {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_stratifier.push(path.with_index("stratifier", index, |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(value, path)?,
                        path,
                    )
                })?);
            }
        }
        Ok(Self {
            id: field_id,
            extension: field_extension,
            modifier_extension: field_modifier_extension,
            link_id: field_link_id,
            title: field_title,
            calculated_date: field_calculated_date,
            code: field_code,
            description: field_description,
            r#type: field_type,
            subject: field_subject,
            scoring: field_scoring,
            composite_scoring: field_composite_scoring,
            improvement_notation: field_improvement_notation,
            improvement_notation_guidance: field_improvement_notation_guidance,
            population: field_population,
            measure_score: field_measure_score,
            stratifier: field_stratifier,
        })
    }
}

impl serde::Serialize for MeasureReportGroup {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        let mut map = serde::Serializer::serialize_map(serializer, None)?;
        if let Some(item) = &self.calculated_date {
            super::super::codec::element_entry(&mut map, "_calculatedDate", item)?;
        }
        if let Some(item) = &self.description {
            super::super::codec::element_entry(&mut map, "_description", item)?;
        }
        if let Some(item) = &self.improvement_notation_guidance {
            super::super::codec::element_entry(&mut map, "_improvementNotationGuidance", item)?;
        }
        if let Some(item) = &self.link_id {
            super::super::codec::element_entry(&mut map, "_linkId", item)?;
        }
        match &self.measure_score {
            Some(MeasureReportGroupMeasureScore::Boolean(inner)) => {
                super::super::codec::element_entry(&mut map, "_measureScoreBoolean", inner)?;
            }
            Some(MeasureReportGroupMeasureScore::DateTime(inner)) => {
                super::super::codec::element_entry(&mut map, "_measureScoreDateTime", inner)?;
            }
            _ => {}
        }
        if let Some(item) = &self.title {
            super::super::codec::element_entry(&mut map, "_title", item)?;
        }
        if let Some(item) = &self.calculated_date {
            super::super::codec::value_entry(&mut map, "calculatedDate", item)?;
        }
        if let Some(item) = &self.code {
            serde::ser::SerializeMap::serialize_entry(&mut map, "code", item)?;
        }
        if let Some(item) = &self.composite_scoring {
            serde::ser::SerializeMap::serialize_entry(&mut map, "compositeScoring", item)?;
        }
        if let Some(item) = &self.description {
            super::super::codec::value_entry(&mut map, "description", item)?;
        }
        if !self.extension.is_empty() {
            serde::ser::SerializeMap::serialize_entry(&mut map, "extension", &self.extension)?;
        }
        if let Some(v) = &self.id {
            serde::ser::SerializeMap::serialize_entry(&mut map, "id", v)?;
        }
        if let Some(item) = &self.improvement_notation {
            serde::ser::SerializeMap::serialize_entry(&mut map, "improvementNotation", item)?;
        }
        if let Some(item) = &self.improvement_notation_guidance {
            super::super::codec::value_entry(&mut map, "improvementNotationGuidance", item)?;
        }
        if let Some(item) = &self.link_id {
            super::super::codec::value_entry(&mut map, "linkId", item)?;
        }
        match &self.measure_score {
            Some(MeasureReportGroupMeasureScore::Boolean(inner)) => {
                super::super::codec::value_entry(&mut map, "measureScoreBoolean", inner)?;
            }
            Some(MeasureReportGroupMeasureScore::CodeableConcept(inner)) => {
                serde::ser::SerializeMap::serialize_entry(
                    &mut map,
                    "measureScoreCodeableConcept",
                    inner.as_ref(),
                )?;
            }
            Some(MeasureReportGroupMeasureScore::DateTime(inner)) => {
                super::super::codec::value_entry(&mut map, "measureScoreDateTime", inner)?;
            }
            Some(MeasureReportGroupMeasureScore::Duration(inner)) => {
                serde::ser::SerializeMap::serialize_entry(
                    &mut map,
                    "measureScoreDuration",
                    inner.as_ref(),
                )?;
            }
            Some(MeasureReportGroupMeasureScore::Period(inner)) => {
                serde::ser::SerializeMap::serialize_entry(
                    &mut map,
                    "measureScorePeriod",
                    inner.as_ref(),
                )?;
            }
            Some(MeasureReportGroupMeasureScore::Quantity(inner)) => {
                serde::ser::SerializeMap::serialize_entry(
                    &mut map,
                    "measureScoreQuantity",
                    inner.as_ref(),
                )?;
            }
            Some(MeasureReportGroupMeasureScore::Range(inner)) => {
                serde::ser::SerializeMap::serialize_entry(
                    &mut map,
                    "measureScoreRange",
                    inner.as_ref(),
                )?;
            }
            Some(MeasureReportGroupMeasureScore::Ratio(inner)) => {
                serde::ser::SerializeMap::serialize_entry(
                    &mut map,
                    "measureScoreRatio",
                    inner.as_ref(),
                )?;
            }
            None => {}
        }
        if !self.modifier_extension.is_empty() {
            serde::ser::SerializeMap::serialize_entry(
                &mut map,
                "modifierExtension",
                &self.modifier_extension,
            )?;
        }
        if !self.population.is_empty() {
            serde::ser::SerializeMap::serialize_entry(&mut map, "population", &self.population)?;
        }
        if let Some(item) = &self.scoring {
            serde::ser::SerializeMap::serialize_entry(&mut map, "scoring", item)?;
        }
        if !self.stratifier.is_empty() {
            serde::ser::SerializeMap::serialize_entry(&mut map, "stratifier", &self.stratifier)?;
        }
        if let Some(item) = &self.subject {
            serde::ser::SerializeMap::serialize_entry(&mut map, "subject", item)?;
        }
        if let Some(item) = &self.title {
            super::super::codec::value_entry(&mut map, "title", item)?;
        }
        if let Some(item) = &self.r#type {
            serde::ser::SerializeMap::serialize_entry(&mut map, "type", item)?;
        }
        serde::ser::SerializeMap::end(map)
    }
}

impl<'de> serde::Deserialize<'de> for MeasureReportGroup {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let value = <super::super::codec::Value as serde::Deserialize>::deserialize(deserializer)?;
        let mut path = super::super::codec::Path::root("MeasureReportGroup");
        let object =
            super::super::codec::expect_object(&value, &path).map_err(serde::de::Error::custom)?;
        super::super::codec::Json::from_json(object, &mut path).map_err(serde::de::Error::custom)
    }
}

/// The \`measureScore\[x\]\` choice of \`MeasureReportGroup\`.
///
/// The measure score for this population group, calculated as appropriate for
/// the measure type and scoring method, and based on the contents of the
/// populations defined in the group.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum MeasureReportGroupMeasureScore {
    /// The `Quantity` form.
    Quantity(Box<super::quantity::Quantity>),
    /// The `dateTime` form.
    DateTime(super::primitives::DateTime),
    /// The `CodeableConcept` form.
    CodeableConcept(Box<super::codeable_concept::CodeableConcept>),
    /// The `Period` form.
    Period(Box<super::period::Period>),
    /// The `Range` form.
    Range(Box<super::range::Range>),
    /// The `Duration` form.
    Duration(Box<super::duration::Duration>),
    /// The `boolean` form.
    Boolean(super::primitives::Boolean),
    /// The `Ratio` form.
    Ratio(Box<super::ratio::Ratio>),
}

impl MeasureReportGroupMeasureScore {
    /// The key suffix, the value part, and the `_name` part of this form.
    ///
    /// # Errors
    ///
    /// Returns [`super::super::codec::EncodeError`] when a held value has no JSON form.
    pub fn to_json_parts(
        &self,
    ) -> Result<
        (
            &'static str,
            Option<super::super::codec::Value>,
            Option<super::super::codec::Value>,
        ),
        super::super::codec::EncodeError,
    > {
        match self {
            Self::Quantity(inner) => Ok((
                "Quantity",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::DateTime(inner) => Ok((
                "DateTime",
                super::super::codec::Primitive::value_json(inner)?,
                super::super::codec::Primitive::element_json(inner)?,
            )),
            Self::CodeableConcept(inner) => Ok((
                "CodeableConcept",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::Period(inner) => Ok((
                "Period",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::Range(inner) => Ok((
                "Range",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::Duration(inner) => Ok((
                "Duration",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::Boolean(inner) => Ok((
                "Boolean",
                super::super::codec::Primitive::value_json(inner)?,
                super::super::codec::Primitive::element_json(inner)?,
            )),
            Self::Ratio(inner) => Ok((
                "Ratio",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
        }
    }

    /// Decodes the form named by `suffix` from its value and `_name` parts.
    ///
    /// # Errors
    ///
    /// Returns [`super::super::codec::DecodeError`] for an unknown suffix or a malformed part.
    pub fn from_json_parts(
        suffix: &str,
        value: Option<&super::super::codec::Value>,
        element: Option<&super::super::codec::Value>,
        path: &mut super::super::codec::Path,
    ) -> Result<Self, super::super::codec::DecodeError> {
        match suffix {
            "Quantity" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::Quantity(Box::new(inner)))
            }
            "DateTime" => Ok(Self::DateTime(
                super::super::codec::Primitive::from_json_parts(value, element, path)?,
            )),
            "CodeableConcept" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::CodeableConcept(Box::new(inner)))
            }
            "Period" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::Period(Box::new(inner)))
            }
            "Range" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::Range(Box::new(inner)))
            }
            "Duration" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::Duration(Box::new(inner)))
            }
            "Boolean" => Ok(Self::Boolean(
                super::super::codec::Primitive::from_json_parts(value, element, path)?,
            )),
            "Ratio" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::Ratio(Box::new(inner)))
            }
            _ => Err(path.error(super::super::codec::DecodeErrorKind::UnknownProperty)),
        }
    }
}

/// The populations in the group
///
/// The populations that make up the population group, one for each type of
/// population appropriate for the measure. Each group in the MeasureReport
/// SHALL have populations as defined in the corresponding group of the Measure
/// being reported.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct MeasureReportGroupPopulation {
    /// Unique id for inter-element referencing
    ///
    /// Unique id for the element within a resource (for internal references). This
    /// may be any string value that does not contain spaces.
    pub id: Option<std::string::String>,
    /// Additional content defined by implementations
    ///
    /// May be used to represent additional information that is not part of the
    /// basic definition of the element. To make the use of extensions safe and
    /// managable, there is a strict set of governance applied to the definition and
    /// use of extensions. Though any implementer can define an extension, there is
    /// a set of requirements that SHALL be met as part of the definition of the
    /// extension.
    pub extension: Vec<super::extension::Extension>,
    /// Extensions that cannot be ignored even if unrecognized
    ///
    /// May be used to represent additional information that is not part of the
    /// basic definition of the element and that modifies the understanding of the
    /// element in which it is contained and/or the understanding of the containing
    /// element's descendants. Usually modifier elements provide negation or
    /// qualification. To make the use of extensions safe and managable, there is a
    /// strict set of governance applied to the definition and use of extensions.
    /// Though any implementer can define an extension, there is a set of
    /// requirements that SHALL be met as part of the definition of the extension.
    /// Applications processing a resource are required to check for modifier
    /// extensions. Modifier extensions SHALL NOT change the meaning of any elements
    /// on Resource or DomainResource (including cannot change the meaning of
    /// modifierExtension itself).
    pub modifier_extension: Vec<super::extension::Extension>,
    /// Pointer to specific population from Measure
    ///
    /// The population from the Measure that corresponds to this population in the
    /// MeasureReport resource. This element SHALL be populated based on the
    /// corresponding element in the Measure being reported.
    pub link_id: Option<super::primitives::String>,
    /// Title of a group. Note- this value is copied from this element in Measure
    ///
    /// Indicates the title for of the group's population.
    pub title: Option<super::primitives::String>,
    /// initial-population | numerator | numerator-exclusion | denominator |
    /// denominator-exclusion | denominator-exception | measure-population |
    /// measure-population-exclusion | measure-observation
    ///
    /// The type of the population. This element SHALL be populated with at least
    /// the codings in the code element of the corresponding population in the
    /// Measure group being reported.
    pub code: Option<super::codeable_concept::CodeableConcept>,
    /// The human readable description of this population criteria
    ///
    /// The human readable description of this population criteria. This element
    /// SHOULD be populated based on the description element of the corresponding
    /// population in the Measure group being reported.
    pub description: Option<super::primitives::Markdown>,
    /// Size of the population
    ///
    /// The number of members of the population.
    pub count: Option<super::primitives::Integer>,
    /// Size of the population as a quantity
    ///
    /// The number of members of the population, specified as a quantity to support
    /// identifying units, as well as to support some composite measure calculation
    /// use cases where the resulting count of the population is a decimal value.
    pub count_quantity: Option<super::quantity::Quantity>,
    /// For subject-list reports, the subject results in this population
    ///
    /// This element refers to a List of individual level MeasureReport resources,
    /// one for each subject in this population.
    pub subject_results: Option<super::reference::Reference>,
    /// For subject-list reports, references to the individual reports for subjects
    /// in this population
    ///
    /// A reference to an individual level MeasureReport resource for a member of
    /// the population.
    pub subject_report: Vec<super::reference::Reference>,
    /// What individual(s) in the population
    ///
    /// Optional Group identifying the individuals that make up the population.
    pub subjects: Option<super::reference::Reference>,
}

impl super::super::codec::Json for MeasureReportGroupPopulation {
    fn to_json(&self) -> Result<super::super::codec::Object, super::super::codec::EncodeError> {
        let mut object = super::super::codec::Object::new();
        if let Some(v) = &self.id {
            object.insert(
                std::string::String::from("id"),
                super::super::codec::Value::String(v.clone()),
            );
        }
        if !self.extension.is_empty() {
            let mut items = Vec::with_capacity(self.extension.len());
            for item in &self.extension {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("extension"),
                super::super::codec::Value::Array(items),
            );
        }
        if !self.modifier_extension.is_empty() {
            let mut items = Vec::with_capacity(self.modifier_extension.len());
            for item in &self.modifier_extension {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("modifierExtension"),
                super::super::codec::Value::Array(items),
            );
        }
        if let Some(item) = &self.link_id {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("linkId"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_linkId"), e);
            }
        }
        if let Some(item) = &self.title {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("title"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_title"), e);
            }
        }
        if let Some(item) = &self.code {
            object.insert(
                std::string::String::from("code"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if let Some(item) = &self.description {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("description"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_description"), e);
            }
        }
        if let Some(item) = &self.count {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("count"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_count"), e);
            }
        }
        if let Some(item) = &self.count_quantity {
            object.insert(
                std::string::String::from("countQuantity"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if let Some(item) = &self.subject_results {
            object.insert(
                std::string::String::from("subjectResults"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if !self.subject_report.is_empty() {
            let mut items = Vec::with_capacity(self.subject_report.len());
            for item in &self.subject_report {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("subjectReport"),
                super::super::codec::Value::Array(items),
            );
        }
        if let Some(item) = &self.subjects {
            object.insert(
                std::string::String::from("subjects"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        Ok(object)
    }

    fn from_json(
        object: &super::super::codec::Object,
        path: &mut super::super::codec::Path,
    ) -> Result<Self, super::super::codec::DecodeError> {
        let mut raw_id: Option<&super::super::codec::Value> = None;
        let mut raw_extension: Option<&super::super::codec::Value> = None;
        let mut raw_modifier_extension: Option<&super::super::codec::Value> = None;
        let mut raw_link_id: Option<&super::super::codec::Value> = None;
        let mut raw_link_id_element: Option<&super::super::codec::Value> = None;
        let mut raw_title: Option<&super::super::codec::Value> = None;
        let mut raw_title_element: Option<&super::super::codec::Value> = None;
        let mut raw_code: Option<&super::super::codec::Value> = None;
        let mut raw_description: Option<&super::super::codec::Value> = None;
        let mut raw_description_element: Option<&super::super::codec::Value> = None;
        let mut raw_count: Option<&super::super::codec::Value> = None;
        let mut raw_count_element: Option<&super::super::codec::Value> = None;
        let mut raw_count_quantity: Option<&super::super::codec::Value> = None;
        let mut raw_subject_results: Option<&super::super::codec::Value> = None;
        let mut raw_subject_report: Option<&super::super::codec::Value> = None;
        let mut raw_subjects: Option<&super::super::codec::Value> = None;
        for (key, value) in object {
            match key.as_str() {
                "id" => {
                    raw_id = Some(value);
                }
                "extension" => {
                    raw_extension = Some(value);
                }
                "modifierExtension" => {
                    raw_modifier_extension = Some(value);
                }
                "linkId" => {
                    raw_link_id = Some(value);
                }
                "_linkId" => {
                    raw_link_id_element = Some(value);
                }
                "title" => {
                    raw_title = Some(value);
                }
                "_title" => {
                    raw_title_element = Some(value);
                }
                "code" => {
                    raw_code = Some(value);
                }
                "description" => {
                    raw_description = Some(value);
                }
                "_description" => {
                    raw_description_element = Some(value);
                }
                "count" => {
                    raw_count = Some(value);
                }
                "_count" => {
                    raw_count_element = Some(value);
                }
                "countQuantity" => {
                    raw_count_quantity = Some(value);
                }
                "subjectResults" => {
                    raw_subject_results = Some(value);
                }
                "subjectReport" => {
                    raw_subject_report = Some(value);
                }
                "subjects" => {
                    raw_subjects = Some(value);
                }
                other => {
                    return path.with(other, |path| {
                        Err(path.error(super::super::codec::DecodeErrorKind::UnknownProperty))
                    });
                }
            }
        }
        let field_id = raw_id
            .map(|value| {
                path.with("id", |path| {
                    let value = super::super::codec::expect_single(value, path)?;
                    super::primitives::checked_string(
                        super::super::codec::expect_string(value, path)?,
                        path,
                    )
                })
            })
            .transpose()?;
        let mut field_extension = Vec::new();
        if let Some(raw) = raw_extension {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_extension.push(path.with_index("extension", index, |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(value, path)?,
                        path,
                    )
                })?);
            }
        }
        let mut field_modifier_extension = Vec::new();
        if let Some(raw) = raw_modifier_extension {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_modifier_extension.push(path.with_index(
                    "modifierExtension",
                    index,
                    |path| {
                        super::super::codec::Json::from_json(
                            super::super::codec::expect_object(value, path)?,
                            path,
                        )
                    },
                )?);
            }
        }
        let field_link_id = match (raw_link_id, raw_link_id_element) {
            (None, None) => None,
            (value, element) => Some(path.with("linkId", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_title = match (raw_title, raw_title_element) {
            (None, None) => None,
            (value, element) => Some(path.with("title", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_code = raw_code
            .map(|value| {
                path.with("code", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let field_description = match (raw_description, raw_description_element) {
            (None, None) => None,
            (value, element) => Some(path.with("description", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_count = match (raw_count, raw_count_element) {
            (None, None) => None,
            (value, element) => Some(path.with("count", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_count_quantity = raw_count_quantity
            .map(|value| {
                path.with("countQuantity", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let field_subject_results = raw_subject_results
            .map(|value| {
                path.with("subjectResults", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let mut field_subject_report = Vec::new();
        if let Some(raw) = raw_subject_report {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_subject_report.push(path.with_index("subjectReport", index, |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(value, path)?,
                        path,
                    )
                })?);
            }
        }
        let field_subjects = raw_subjects
            .map(|value| {
                path.with("subjects", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        Ok(Self {
            id: field_id,
            extension: field_extension,
            modifier_extension: field_modifier_extension,
            link_id: field_link_id,
            title: field_title,
            code: field_code,
            description: field_description,
            count: field_count,
            count_quantity: field_count_quantity,
            subject_results: field_subject_results,
            subject_report: field_subject_report,
            subjects: field_subjects,
        })
    }
}

impl serde::Serialize for MeasureReportGroupPopulation {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        let mut map = serde::Serializer::serialize_map(serializer, None)?;
        if let Some(item) = &self.count {
            super::super::codec::element_entry(&mut map, "_count", item)?;
        }
        if let Some(item) = &self.description {
            super::super::codec::element_entry(&mut map, "_description", item)?;
        }
        if let Some(item) = &self.link_id {
            super::super::codec::element_entry(&mut map, "_linkId", item)?;
        }
        if let Some(item) = &self.title {
            super::super::codec::element_entry(&mut map, "_title", item)?;
        }
        if let Some(item) = &self.code {
            serde::ser::SerializeMap::serialize_entry(&mut map, "code", item)?;
        }
        if let Some(item) = &self.count {
            super::super::codec::value_entry(&mut map, "count", item)?;
        }
        if let Some(item) = &self.count_quantity {
            serde::ser::SerializeMap::serialize_entry(&mut map, "countQuantity", item)?;
        }
        if let Some(item) = &self.description {
            super::super::codec::value_entry(&mut map, "description", item)?;
        }
        if !self.extension.is_empty() {
            serde::ser::SerializeMap::serialize_entry(&mut map, "extension", &self.extension)?;
        }
        if let Some(v) = &self.id {
            serde::ser::SerializeMap::serialize_entry(&mut map, "id", v)?;
        }
        if let Some(item) = &self.link_id {
            super::super::codec::value_entry(&mut map, "linkId", item)?;
        }
        if !self.modifier_extension.is_empty() {
            serde::ser::SerializeMap::serialize_entry(
                &mut map,
                "modifierExtension",
                &self.modifier_extension,
            )?;
        }
        if !self.subject_report.is_empty() {
            serde::ser::SerializeMap::serialize_entry(
                &mut map,
                "subjectReport",
                &self.subject_report,
            )?;
        }
        if let Some(item) = &self.subject_results {
            serde::ser::SerializeMap::serialize_entry(&mut map, "subjectResults", item)?;
        }
        if let Some(item) = &self.subjects {
            serde::ser::SerializeMap::serialize_entry(&mut map, "subjects", item)?;
        }
        if let Some(item) = &self.title {
            super::super::codec::value_entry(&mut map, "title", item)?;
        }
        serde::ser::SerializeMap::end(map)
    }
}

impl<'de> serde::Deserialize<'de> for MeasureReportGroupPopulation {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let value = <super::super::codec::Value as serde::Deserialize>::deserialize(deserializer)?;
        let mut path = super::super::codec::Path::root("MeasureReportGroupPopulation");
        let object =
            super::super::codec::expect_object(&value, &path).map_err(serde::de::Error::custom)?;
        super::super::codec::Json::from_json(object, &mut path).map_err(serde::de::Error::custom)
    }
}

/// Stratification results
///
/// The stratification results for this measure group, calculated as defined by
/// the stratifier element of the measure being reported. Each group in the
/// MeasureReport SHALL have stratifiers as defined in the corresponding group
/// of the Measure being reported.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct MeasureReportGroupStratifier {
    /// Unique id for inter-element referencing
    ///
    /// Unique id for the element within a resource (for internal references). This
    /// may be any string value that does not contain spaces.
    pub id: Option<std::string::String>,
    /// Additional content defined by implementations
    ///
    /// May be used to represent additional information that is not part of the
    /// basic definition of the element. To make the use of extensions safe and
    /// managable, there is a strict set of governance applied to the definition and
    /// use of extensions. Though any implementer can define an extension, there is
    /// a set of requirements that SHALL be met as part of the definition of the
    /// extension.
    pub extension: Vec<super::extension::Extension>,
    /// Extensions that cannot be ignored even if unrecognized
    ///
    /// May be used to represent additional information that is not part of the
    /// basic definition of the element and that modifies the understanding of the
    /// element in which it is contained and/or the understanding of the containing
    /// element's descendants. Usually modifier elements provide negation or
    /// qualification. To make the use of extensions safe and managable, there is a
    /// strict set of governance applied to the definition and use of extensions.
    /// Though any implementer can define an extension, there is a set of
    /// requirements that SHALL be met as part of the definition of the extension.
    /// Applications processing a resource are required to check for modifier
    /// extensions. Modifier extensions SHALL NOT change the meaning of any elements
    /// on Resource or DomainResource (including cannot change the meaning of
    /// modifierExtension itself).
    pub modifier_extension: Vec<super::extension::Extension>,
    /// Pointer to specific stratifier from Measure
    ///
    /// The stratifier from the Measure that corresponds to this stratifier in the
    /// MeasureReport resource. This element SHALL be populated based on the
    /// corresponding element in the Measure being reported.
    pub link_id: Option<super::primitives::String>,
    /// Title of a group's stratifier. Note- this value is copied from this element
    /// in Measure
    ///
    /// Indicates the title for of the group's stratifier.
    pub title: Option<super::primitives::String>,
    /// What stratifier of the group
    ///
    /// The meaning of this stratifier, as defined in the measure definition. This
    /// element SHALL be populated with at least the codings in the code element of
    /// the corresponding stratifier in the Measure group being reported.
    pub code: Option<super::codeable_concept::CodeableConcept>,
    /// The human readable description of this stratifier
    ///
    /// The human readable description of this stratifier criteria. This element
    /// SHOULD be populated with the description element of the corresponding
    /// stratifier in the Measure group being reported.
    pub description: Option<super::primitives::Markdown>,
    /// Stratum results, one for each unique value, or set of values, in the
    /// stratifier, or stratifier components
    ///
    /// This element contains the results for a single stratum within the
    /// stratifier. For example, when stratifying on administrative gender, there
    /// will be four strata, one for each possible gender value.
    pub stratum: Vec<MeasureReportGroupStratifierStratum>,
}

impl super::super::codec::Json for MeasureReportGroupStratifier {
    fn to_json(&self) -> Result<super::super::codec::Object, super::super::codec::EncodeError> {
        let mut object = super::super::codec::Object::new();
        if let Some(v) = &self.id {
            object.insert(
                std::string::String::from("id"),
                super::super::codec::Value::String(v.clone()),
            );
        }
        if !self.extension.is_empty() {
            let mut items = Vec::with_capacity(self.extension.len());
            for item in &self.extension {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("extension"),
                super::super::codec::Value::Array(items),
            );
        }
        if !self.modifier_extension.is_empty() {
            let mut items = Vec::with_capacity(self.modifier_extension.len());
            for item in &self.modifier_extension {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("modifierExtension"),
                super::super::codec::Value::Array(items),
            );
        }
        if let Some(item) = &self.link_id {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("linkId"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_linkId"), e);
            }
        }
        if let Some(item) = &self.title {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("title"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_title"), e);
            }
        }
        if let Some(item) = &self.code {
            object.insert(
                std::string::String::from("code"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if let Some(item) = &self.description {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("description"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_description"), e);
            }
        }
        if !self.stratum.is_empty() {
            let mut items = Vec::with_capacity(self.stratum.len());
            for item in &self.stratum {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("stratum"),
                super::super::codec::Value::Array(items),
            );
        }
        Ok(object)
    }

    fn from_json(
        object: &super::super::codec::Object,
        path: &mut super::super::codec::Path,
    ) -> Result<Self, super::super::codec::DecodeError> {
        let mut raw_id: Option<&super::super::codec::Value> = None;
        let mut raw_extension: Option<&super::super::codec::Value> = None;
        let mut raw_modifier_extension: Option<&super::super::codec::Value> = None;
        let mut raw_link_id: Option<&super::super::codec::Value> = None;
        let mut raw_link_id_element: Option<&super::super::codec::Value> = None;
        let mut raw_title: Option<&super::super::codec::Value> = None;
        let mut raw_title_element: Option<&super::super::codec::Value> = None;
        let mut raw_code: Option<&super::super::codec::Value> = None;
        let mut raw_description: Option<&super::super::codec::Value> = None;
        let mut raw_description_element: Option<&super::super::codec::Value> = None;
        let mut raw_stratum: Option<&super::super::codec::Value> = None;
        for (key, value) in object {
            match key.as_str() {
                "id" => {
                    raw_id = Some(value);
                }
                "extension" => {
                    raw_extension = Some(value);
                }
                "modifierExtension" => {
                    raw_modifier_extension = Some(value);
                }
                "linkId" => {
                    raw_link_id = Some(value);
                }
                "_linkId" => {
                    raw_link_id_element = Some(value);
                }
                "title" => {
                    raw_title = Some(value);
                }
                "_title" => {
                    raw_title_element = Some(value);
                }
                "code" => {
                    raw_code = Some(value);
                }
                "description" => {
                    raw_description = Some(value);
                }
                "_description" => {
                    raw_description_element = Some(value);
                }
                "stratum" => {
                    raw_stratum = Some(value);
                }
                other => {
                    return path.with(other, |path| {
                        Err(path.error(super::super::codec::DecodeErrorKind::UnknownProperty))
                    });
                }
            }
        }
        let field_id = raw_id
            .map(|value| {
                path.with("id", |path| {
                    let value = super::super::codec::expect_single(value, path)?;
                    super::primitives::checked_string(
                        super::super::codec::expect_string(value, path)?,
                        path,
                    )
                })
            })
            .transpose()?;
        let mut field_extension = Vec::new();
        if let Some(raw) = raw_extension {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_extension.push(path.with_index("extension", index, |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(value, path)?,
                        path,
                    )
                })?);
            }
        }
        let mut field_modifier_extension = Vec::new();
        if let Some(raw) = raw_modifier_extension {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_modifier_extension.push(path.with_index(
                    "modifierExtension",
                    index,
                    |path| {
                        super::super::codec::Json::from_json(
                            super::super::codec::expect_object(value, path)?,
                            path,
                        )
                    },
                )?);
            }
        }
        let field_link_id = match (raw_link_id, raw_link_id_element) {
            (None, None) => None,
            (value, element) => Some(path.with("linkId", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_title = match (raw_title, raw_title_element) {
            (None, None) => None,
            (value, element) => Some(path.with("title", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_code = raw_code
            .map(|value| {
                path.with("code", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let field_description = match (raw_description, raw_description_element) {
            (None, None) => None,
            (value, element) => Some(path.with("description", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let mut field_stratum = Vec::new();
        if let Some(raw) = raw_stratum {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_stratum.push(path.with_index("stratum", index, |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(value, path)?,
                        path,
                    )
                })?);
            }
        }
        Ok(Self {
            id: field_id,
            extension: field_extension,
            modifier_extension: field_modifier_extension,
            link_id: field_link_id,
            title: field_title,
            code: field_code,
            description: field_description,
            stratum: field_stratum,
        })
    }
}

impl serde::Serialize for MeasureReportGroupStratifier {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        let mut map = serde::Serializer::serialize_map(serializer, None)?;
        if let Some(item) = &self.description {
            super::super::codec::element_entry(&mut map, "_description", item)?;
        }
        if let Some(item) = &self.link_id {
            super::super::codec::element_entry(&mut map, "_linkId", item)?;
        }
        if let Some(item) = &self.title {
            super::super::codec::element_entry(&mut map, "_title", item)?;
        }
        if let Some(item) = &self.code {
            serde::ser::SerializeMap::serialize_entry(&mut map, "code", item)?;
        }
        if let Some(item) = &self.description {
            super::super::codec::value_entry(&mut map, "description", item)?;
        }
        if !self.extension.is_empty() {
            serde::ser::SerializeMap::serialize_entry(&mut map, "extension", &self.extension)?;
        }
        if let Some(v) = &self.id {
            serde::ser::SerializeMap::serialize_entry(&mut map, "id", v)?;
        }
        if let Some(item) = &self.link_id {
            super::super::codec::value_entry(&mut map, "linkId", item)?;
        }
        if !self.modifier_extension.is_empty() {
            serde::ser::SerializeMap::serialize_entry(
                &mut map,
                "modifierExtension",
                &self.modifier_extension,
            )?;
        }
        if !self.stratum.is_empty() {
            serde::ser::SerializeMap::serialize_entry(&mut map, "stratum", &self.stratum)?;
        }
        if let Some(item) = &self.title {
            super::super::codec::value_entry(&mut map, "title", item)?;
        }
        serde::ser::SerializeMap::end(map)
    }
}

impl<'de> serde::Deserialize<'de> for MeasureReportGroupStratifier {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let value = <super::super::codec::Value as serde::Deserialize>::deserialize(deserializer)?;
        let mut path = super::super::codec::Path::root("MeasureReportGroupStratifier");
        let object =
            super::super::codec::expect_object(&value, &path).map_err(serde::de::Error::custom)?;
        super::super::codec::Json::from_json(object, &mut path).map_err(serde::de::Error::custom)
    }
}

/// Stratum results, one for each unique value, or set of values, in the
/// stratifier, or stratifier components
///
/// This element contains the results for a single stratum within the
/// stratifier. For example, when stratifying on administrative gender, there
/// will be four strata, one for each possible gender value.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct MeasureReportGroupStratifierStratum {
    /// Unique id for inter-element referencing
    ///
    /// Unique id for the element within a resource (for internal references). This
    /// may be any string value that does not contain spaces.
    pub id: Option<std::string::String>,
    /// Additional content defined by implementations
    ///
    /// May be used to represent additional information that is not part of the
    /// basic definition of the element. To make the use of extensions safe and
    /// managable, there is a strict set of governance applied to the definition and
    /// use of extensions. Though any implementer can define an extension, there is
    /// a set of requirements that SHALL be met as part of the definition of the
    /// extension.
    pub extension: Vec<super::extension::Extension>,
    /// Extensions that cannot be ignored even if unrecognized
    ///
    /// May be used to represent additional information that is not part of the
    /// basic definition of the element and that modifies the understanding of the
    /// element in which it is contained and/or the understanding of the containing
    /// element's descendants. Usually modifier elements provide negation or
    /// qualification. To make the use of extensions safe and managable, there is a
    /// strict set of governance applied to the definition and use of extensions.
    /// Though any implementer can define an extension, there is a set of
    /// requirements that SHALL be met as part of the definition of the extension.
    /// Applications processing a resource are required to check for modifier
    /// extensions. Modifier extensions SHALL NOT change the meaning of any elements
    /// on Resource or DomainResource (including cannot change the meaning of
    /// modifierExtension itself).
    pub modifier_extension: Vec<super::extension::Extension>,
    /// The stratum value, e.g. male
    ///
    /// The value for this stratum, expressed as a CodeableConcept. When defining
    /// stratifiers on complex values, the value must be rendered such that the
    /// value for each stratum within the stratifier is unique.
    pub value: Option<MeasureReportGroupStratifierStratumValue>,
    /// Stratifier component values
    ///
    /// A stratifier component value.
    pub component: Vec<MeasureReportGroupStratifierStratumComponent>,
    /// Population results in this stratum
    ///
    /// The populations that make up the stratum, one for each type of population
    /// appropriate to the measure. For each stratifier, systems MAY provide
    /// population breakdowns in addition to the stratified scores.
    pub population: Vec<MeasureReportGroupStratifierStratumPopulation>,
    /// What score this stratum achieved
    ///
    /// The measure score for this stratum, calculated as appropriate for the
    /// measure type and scoring method, and based on only the members of this
    /// stratum.
    pub measure_score: Option<MeasureReportGroupStratifierStratumMeasureScore>,
}

impl super::super::codec::Json for MeasureReportGroupStratifierStratum {
    fn to_json(&self) -> Result<super::super::codec::Object, super::super::codec::EncodeError> {
        let mut object = super::super::codec::Object::new();
        if let Some(v) = &self.id {
            object.insert(
                std::string::String::from("id"),
                super::super::codec::Value::String(v.clone()),
            );
        }
        if !self.extension.is_empty() {
            let mut items = Vec::with_capacity(self.extension.len());
            for item in &self.extension {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("extension"),
                super::super::codec::Value::Array(items),
            );
        }
        if !self.modifier_extension.is_empty() {
            let mut items = Vec::with_capacity(self.modifier_extension.len());
            for item in &self.modifier_extension {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("modifierExtension"),
                super::super::codec::Value::Array(items),
            );
        }
        if let Some(item) = &self.value {
            let (suffix, value, element) = item.to_json_parts()?;
            if let Some(value) = value {
                object.insert(format!("value{suffix}"), value);
            }
            if let Some(element) = element {
                object.insert(format!("_value{suffix}"), element);
            }
        }
        if !self.component.is_empty() {
            let mut items = Vec::with_capacity(self.component.len());
            for item in &self.component {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("component"),
                super::super::codec::Value::Array(items),
            );
        }
        if !self.population.is_empty() {
            let mut items = Vec::with_capacity(self.population.len());
            for item in &self.population {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("population"),
                super::super::codec::Value::Array(items),
            );
        }
        if let Some(item) = &self.measure_score {
            let (suffix, value, element) = item.to_json_parts()?;
            if let Some(value) = value {
                object.insert(format!("measureScore{suffix}"), value);
            }
            if let Some(element) = element {
                object.insert(format!("_measureScore{suffix}"), element);
            }
        }
        Ok(object)
    }

    fn from_json(
        object: &super::super::codec::Object,
        path: &mut super::super::codec::Path,
    ) -> Result<Self, super::super::codec::DecodeError> {
        let mut raw_id: Option<&super::super::codec::Value> = None;
        let mut raw_extension: Option<&super::super::codec::Value> = None;
        let mut raw_modifier_extension: Option<&super::super::codec::Value> = None;
        let mut raw_value = super::super::codec::ChoiceSlot::default();
        let mut raw_component: Option<&super::super::codec::Value> = None;
        let mut raw_population: Option<&super::super::codec::Value> = None;
        let mut raw_measure_score = super::super::codec::ChoiceSlot::default();
        for (key, value) in object {
            match key.as_str() {
                "id" => {
                    raw_id = Some(value);
                }
                "extension" => {
                    raw_extension = Some(value);
                }
                "modifierExtension" => {
                    raw_modifier_extension = Some(value);
                }
                "valueCodeableConcept" => {
                    raw_value.value("CodeableConcept", value, path)?;
                }
                "_valueCodeableConcept" => {
                    raw_value.element("CodeableConcept", value, path)?;
                }
                "valueBoolean" => {
                    raw_value.value("Boolean", value, path)?;
                }
                "_valueBoolean" => {
                    raw_value.element("Boolean", value, path)?;
                }
                "valuePeriod" => {
                    raw_value.value("Period", value, path)?;
                }
                "_valuePeriod" => {
                    raw_value.element("Period", value, path)?;
                }
                "valueQuantity" => {
                    raw_value.value("Quantity", value, path)?;
                }
                "_valueQuantity" => {
                    raw_value.element("Quantity", value, path)?;
                }
                "valueRange" => {
                    raw_value.value("Range", value, path)?;
                }
                "_valueRange" => {
                    raw_value.element("Range", value, path)?;
                }
                "valueReference" => {
                    raw_value.value("Reference", value, path)?;
                }
                "_valueReference" => {
                    raw_value.element("Reference", value, path)?;
                }
                "valueString" => {
                    raw_value.value("String", value, path)?;
                }
                "_valueString" => {
                    raw_value.element("String", value, path)?;
                }
                "component" => {
                    raw_component = Some(value);
                }
                "population" => {
                    raw_population = Some(value);
                }
                "measureScoreQuantity" => {
                    raw_measure_score.value("Quantity", value, path)?;
                }
                "_measureScoreQuantity" => {
                    raw_measure_score.element("Quantity", value, path)?;
                }
                "measureScoreDateTime" => {
                    raw_measure_score.value("DateTime", value, path)?;
                }
                "_measureScoreDateTime" => {
                    raw_measure_score.element("DateTime", value, path)?;
                }
                "measureScoreCodeableConcept" => {
                    raw_measure_score.value("CodeableConcept", value, path)?;
                }
                "_measureScoreCodeableConcept" => {
                    raw_measure_score.element("CodeableConcept", value, path)?;
                }
                "measureScorePeriod" => {
                    raw_measure_score.value("Period", value, path)?;
                }
                "_measureScorePeriod" => {
                    raw_measure_score.element("Period", value, path)?;
                }
                "measureScoreRange" => {
                    raw_measure_score.value("Range", value, path)?;
                }
                "_measureScoreRange" => {
                    raw_measure_score.element("Range", value, path)?;
                }
                "measureScoreDuration" => {
                    raw_measure_score.value("Duration", value, path)?;
                }
                "_measureScoreDuration" => {
                    raw_measure_score.element("Duration", value, path)?;
                }
                "measureScoreBoolean" => {
                    raw_measure_score.value("Boolean", value, path)?;
                }
                "_measureScoreBoolean" => {
                    raw_measure_score.element("Boolean", value, path)?;
                }
                "measureScoreRatio" => {
                    raw_measure_score.value("Ratio", value, path)?;
                }
                "_measureScoreRatio" => {
                    raw_measure_score.element("Ratio", value, path)?;
                }
                other => {
                    return path.with(other, |path| {
                        Err(path.error(super::super::codec::DecodeErrorKind::UnknownProperty))
                    });
                }
            }
        }
        let field_id = raw_id
            .map(|value| {
                path.with("id", |path| {
                    let value = super::super::codec::expect_single(value, path)?;
                    super::primitives::checked_string(
                        super::super::codec::expect_string(value, path)?,
                        path,
                    )
                })
            })
            .transpose()?;
        let mut field_extension = Vec::new();
        if let Some(raw) = raw_extension {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_extension.push(path.with_index("extension", index, |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(value, path)?,
                        path,
                    )
                })?);
            }
        }
        let mut field_modifier_extension = Vec::new();
        if let Some(raw) = raw_modifier_extension {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_modifier_extension.push(path.with_index(
                    "modifierExtension",
                    index,
                    |path| {
                        super::super::codec::Json::from_json(
                            super::super::codec::expect_object(value, path)?,
                            path,
                        )
                    },
                )?);
            }
        }
        let field_value = match raw_value.suffix {
            None => None,
            Some(suffix) => Some(path.with("value", |path| {
                MeasureReportGroupStratifierStratumValue::from_json_parts(
                    suffix,
                    raw_value.value,
                    raw_value.element,
                    path,
                )
            })?),
        };
        let mut field_component = Vec::new();
        if let Some(raw) = raw_component {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_component.push(path.with_index("component", index, |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(value, path)?,
                        path,
                    )
                })?);
            }
        }
        let mut field_population = Vec::new();
        if let Some(raw) = raw_population {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_population.push(path.with_index("population", index, |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(value, path)?,
                        path,
                    )
                })?);
            }
        }
        let field_measure_score = match raw_measure_score.suffix {
            None => None,
            Some(suffix) => Some(path.with("measureScore", |path| {
                MeasureReportGroupStratifierStratumMeasureScore::from_json_parts(
                    suffix,
                    raw_measure_score.value,
                    raw_measure_score.element,
                    path,
                )
            })?),
        };
        Ok(Self {
            id: field_id,
            extension: field_extension,
            modifier_extension: field_modifier_extension,
            value: field_value,
            component: field_component,
            population: field_population,
            measure_score: field_measure_score,
        })
    }
}

impl serde::Serialize for MeasureReportGroupStratifierStratum {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        let mut map = serde::Serializer::serialize_map(serializer, None)?;
        match &self.measure_score {
            Some(MeasureReportGroupStratifierStratumMeasureScore::Boolean(inner)) => {
                super::super::codec::element_entry(&mut map, "_measureScoreBoolean", inner)?;
            }
            Some(MeasureReportGroupStratifierStratumMeasureScore::DateTime(inner)) => {
                super::super::codec::element_entry(&mut map, "_measureScoreDateTime", inner)?;
            }
            _ => {}
        }
        match &self.value {
            Some(MeasureReportGroupStratifierStratumValue::Boolean(inner)) => {
                super::super::codec::element_entry(&mut map, "_valueBoolean", inner)?;
            }
            Some(MeasureReportGroupStratifierStratumValue::String(inner)) => {
                super::super::codec::element_entry(&mut map, "_valueString", inner)?;
            }
            _ => {}
        }
        if !self.component.is_empty() {
            serde::ser::SerializeMap::serialize_entry(&mut map, "component", &self.component)?;
        }
        if !self.extension.is_empty() {
            serde::ser::SerializeMap::serialize_entry(&mut map, "extension", &self.extension)?;
        }
        if let Some(v) = &self.id {
            serde::ser::SerializeMap::serialize_entry(&mut map, "id", v)?;
        }
        match &self.measure_score {
            Some(MeasureReportGroupStratifierStratumMeasureScore::Boolean(inner)) => {
                super::super::codec::value_entry(&mut map, "measureScoreBoolean", inner)?;
            }
            Some(MeasureReportGroupStratifierStratumMeasureScore::CodeableConcept(inner)) => {
                serde::ser::SerializeMap::serialize_entry(
                    &mut map,
                    "measureScoreCodeableConcept",
                    inner.as_ref(),
                )?;
            }
            Some(MeasureReportGroupStratifierStratumMeasureScore::DateTime(inner)) => {
                super::super::codec::value_entry(&mut map, "measureScoreDateTime", inner)?;
            }
            Some(MeasureReportGroupStratifierStratumMeasureScore::Duration(inner)) => {
                serde::ser::SerializeMap::serialize_entry(
                    &mut map,
                    "measureScoreDuration",
                    inner.as_ref(),
                )?;
            }
            Some(MeasureReportGroupStratifierStratumMeasureScore::Period(inner)) => {
                serde::ser::SerializeMap::serialize_entry(
                    &mut map,
                    "measureScorePeriod",
                    inner.as_ref(),
                )?;
            }
            Some(MeasureReportGroupStratifierStratumMeasureScore::Quantity(inner)) => {
                serde::ser::SerializeMap::serialize_entry(
                    &mut map,
                    "measureScoreQuantity",
                    inner.as_ref(),
                )?;
            }
            Some(MeasureReportGroupStratifierStratumMeasureScore::Range(inner)) => {
                serde::ser::SerializeMap::serialize_entry(
                    &mut map,
                    "measureScoreRange",
                    inner.as_ref(),
                )?;
            }
            Some(MeasureReportGroupStratifierStratumMeasureScore::Ratio(inner)) => {
                serde::ser::SerializeMap::serialize_entry(
                    &mut map,
                    "measureScoreRatio",
                    inner.as_ref(),
                )?;
            }
            None => {}
        }
        if !self.modifier_extension.is_empty() {
            serde::ser::SerializeMap::serialize_entry(
                &mut map,
                "modifierExtension",
                &self.modifier_extension,
            )?;
        }
        if !self.population.is_empty() {
            serde::ser::SerializeMap::serialize_entry(&mut map, "population", &self.population)?;
        }
        match &self.value {
            Some(MeasureReportGroupStratifierStratumValue::Boolean(inner)) => {
                super::super::codec::value_entry(&mut map, "valueBoolean", inner)?;
            }
            Some(MeasureReportGroupStratifierStratumValue::CodeableConcept(inner)) => {
                serde::ser::SerializeMap::serialize_entry(
                    &mut map,
                    "valueCodeableConcept",
                    inner.as_ref(),
                )?;
            }
            Some(MeasureReportGroupStratifierStratumValue::Period(inner)) => {
                serde::ser::SerializeMap::serialize_entry(&mut map, "valuePeriod", inner.as_ref())?;
            }
            Some(MeasureReportGroupStratifierStratumValue::Quantity(inner)) => {
                serde::ser::SerializeMap::serialize_entry(
                    &mut map,
                    "valueQuantity",
                    inner.as_ref(),
                )?;
            }
            Some(MeasureReportGroupStratifierStratumValue::Range(inner)) => {
                serde::ser::SerializeMap::serialize_entry(&mut map, "valueRange", inner.as_ref())?;
            }
            Some(MeasureReportGroupStratifierStratumValue::Reference(inner)) => {
                serde::ser::SerializeMap::serialize_entry(
                    &mut map,
                    "valueReference",
                    inner.as_ref(),
                )?;
            }
            Some(MeasureReportGroupStratifierStratumValue::String(inner)) => {
                super::super::codec::value_entry(&mut map, "valueString", inner)?;
            }
            None => {}
        }
        serde::ser::SerializeMap::end(map)
    }
}

impl<'de> serde::Deserialize<'de> for MeasureReportGroupStratifierStratum {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let value = <super::super::codec::Value as serde::Deserialize>::deserialize(deserializer)?;
        let mut path = super::super::codec::Path::root("MeasureReportGroupStratifierStratum");
        let object =
            super::super::codec::expect_object(&value, &path).map_err(serde::de::Error::custom)?;
        super::super::codec::Json::from_json(object, &mut path).map_err(serde::de::Error::custom)
    }
}

/// Stratifier component values
///
/// A stratifier component value.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MeasureReportGroupStratifierStratumComponent {
    /// Unique id for inter-element referencing
    ///
    /// Unique id for the element within a resource (for internal references). This
    /// may be any string value that does not contain spaces.
    pub id: Option<std::string::String>,
    /// Additional content defined by implementations
    ///
    /// May be used to represent additional information that is not part of the
    /// basic definition of the element. To make the use of extensions safe and
    /// managable, there is a strict set of governance applied to the definition and
    /// use of extensions. Though any implementer can define an extension, there is
    /// a set of requirements that SHALL be met as part of the definition of the
    /// extension.
    pub extension: Vec<super::extension::Extension>,
    /// Extensions that cannot be ignored even if unrecognized
    ///
    /// May be used to represent additional information that is not part of the
    /// basic definition of the element and that modifies the understanding of the
    /// element in which it is contained and/or the understanding of the containing
    /// element's descendants. Usually modifier elements provide negation or
    /// qualification. To make the use of extensions safe and managable, there is a
    /// strict set of governance applied to the definition and use of extensions.
    /// Though any implementer can define an extension, there is a set of
    /// requirements that SHALL be met as part of the definition of the extension.
    /// Applications processing a resource are required to check for modifier
    /// extensions. Modifier extensions SHALL NOT change the meaning of any elements
    /// on Resource or DomainResource (including cannot change the meaning of
    /// modifierExtension itself).
    pub modifier_extension: Vec<super::extension::Extension>,
    /// Pointer to specific stratifier component from Measure
    ///
    /// The stratifier component from the Measure that corresponds to this
    /// stratifier component in the MeasureReport resource. This element SHALL be
    /// populated based on the corresponding element in the Measure being reported.
    pub link_id: Option<super::primitives::String>,
    /// What stratifier component of the group
    ///
    /// The code for the stratum component value. This element SHALL be populated
    /// with at least the codings in the code element of the corresponding component
    /// of the stratifier being reported.
    pub code: super::codeable_concept::CodeableConcept,
    /// The human readable description of this stratifier component
    ///
    /// The human readable description of this stratifier criteria component. This
    /// element MAY be populated with the description of the corresponding component
    /// of the stratifier being reported.
    pub description: Option<super::primitives::Markdown>,
    /// The stratum component value, e.g. male
    ///
    /// The stratum component value.
    pub value: MeasureReportGroupStratifierStratumComponentValue,
}

impl super::super::codec::Json for MeasureReportGroupStratifierStratumComponent {
    fn to_json(&self) -> Result<super::super::codec::Object, super::super::codec::EncodeError> {
        let mut object = super::super::codec::Object::new();
        if let Some(v) = &self.id {
            object.insert(
                std::string::String::from("id"),
                super::super::codec::Value::String(v.clone()),
            );
        }
        if !self.extension.is_empty() {
            let mut items = Vec::with_capacity(self.extension.len());
            for item in &self.extension {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("extension"),
                super::super::codec::Value::Array(items),
            );
        }
        if !self.modifier_extension.is_empty() {
            let mut items = Vec::with_capacity(self.modifier_extension.len());
            for item in &self.modifier_extension {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("modifierExtension"),
                super::super::codec::Value::Array(items),
            );
        }
        if let Some(item) = &self.link_id {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("linkId"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_linkId"), e);
            }
        }
        object.insert(
            std::string::String::from("code"),
            super::super::codec::Value::Object(super::super::codec::Json::to_json(&self.code)?),
        );
        if let Some(item) = &self.description {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("description"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_description"), e);
            }
        }
        let (suffix, value, element) = self.value.to_json_parts()?;
        if let Some(value) = value {
            object.insert(format!("value{suffix}"), value);
        }
        if let Some(element) = element {
            object.insert(format!("_value{suffix}"), element);
        }
        Ok(object)
    }

    fn from_json(
        object: &super::super::codec::Object,
        path: &mut super::super::codec::Path,
    ) -> Result<Self, super::super::codec::DecodeError> {
        let mut raw_id: Option<&super::super::codec::Value> = None;
        let mut raw_extension: Option<&super::super::codec::Value> = None;
        let mut raw_modifier_extension: Option<&super::super::codec::Value> = None;
        let mut raw_link_id: Option<&super::super::codec::Value> = None;
        let mut raw_link_id_element: Option<&super::super::codec::Value> = None;
        let mut raw_code: Option<&super::super::codec::Value> = None;
        let mut raw_description: Option<&super::super::codec::Value> = None;
        let mut raw_description_element: Option<&super::super::codec::Value> = None;
        let mut raw_value = super::super::codec::ChoiceSlot::default();
        for (key, value) in object {
            match key.as_str() {
                "id" => {
                    raw_id = Some(value);
                }
                "extension" => {
                    raw_extension = Some(value);
                }
                "modifierExtension" => {
                    raw_modifier_extension = Some(value);
                }
                "linkId" => {
                    raw_link_id = Some(value);
                }
                "_linkId" => {
                    raw_link_id_element = Some(value);
                }
                "code" => {
                    raw_code = Some(value);
                }
                "description" => {
                    raw_description = Some(value);
                }
                "_description" => {
                    raw_description_element = Some(value);
                }
                "valueCodeableConcept" => {
                    raw_value.value("CodeableConcept", value, path)?;
                }
                "_valueCodeableConcept" => {
                    raw_value.element("CodeableConcept", value, path)?;
                }
                "valueBoolean" => {
                    raw_value.value("Boolean", value, path)?;
                }
                "_valueBoolean" => {
                    raw_value.element("Boolean", value, path)?;
                }
                "valuePeriod" => {
                    raw_value.value("Period", value, path)?;
                }
                "_valuePeriod" => {
                    raw_value.element("Period", value, path)?;
                }
                "valueQuantity" => {
                    raw_value.value("Quantity", value, path)?;
                }
                "_valueQuantity" => {
                    raw_value.element("Quantity", value, path)?;
                }
                "valueRange" => {
                    raw_value.value("Range", value, path)?;
                }
                "_valueRange" => {
                    raw_value.element("Range", value, path)?;
                }
                "valueReference" => {
                    raw_value.value("Reference", value, path)?;
                }
                "_valueReference" => {
                    raw_value.element("Reference", value, path)?;
                }
                "valueString" => {
                    raw_value.value("String", value, path)?;
                }
                "_valueString" => {
                    raw_value.element("String", value, path)?;
                }
                other => {
                    return path.with(other, |path| {
                        Err(path.error(super::super::codec::DecodeErrorKind::UnknownProperty))
                    });
                }
            }
        }
        let field_id = raw_id
            .map(|value| {
                path.with("id", |path| {
                    let value = super::super::codec::expect_single(value, path)?;
                    super::primitives::checked_string(
                        super::super::codec::expect_string(value, path)?,
                        path,
                    )
                })
            })
            .transpose()?;
        let mut field_extension = Vec::new();
        if let Some(raw) = raw_extension {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_extension.push(path.with_index("extension", index, |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(value, path)?,
                        path,
                    )
                })?);
            }
        }
        let mut field_modifier_extension = Vec::new();
        if let Some(raw) = raw_modifier_extension {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_modifier_extension.push(path.with_index(
                    "modifierExtension",
                    index,
                    |path| {
                        super::super::codec::Json::from_json(
                            super::super::codec::expect_object(value, path)?,
                            path,
                        )
                    },
                )?);
            }
        }
        let field_link_id = match (raw_link_id, raw_link_id_element) {
            (None, None) => None,
            (value, element) => Some(path.with("linkId", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_code = path.with("code", |path| {
            let value = raw_code
                .ok_or_else(|| path.error(super::super::codec::DecodeErrorKind::MissingProperty))?;
            super::super::codec::Json::from_json(
                super::super::codec::expect_object(
                    super::super::codec::expect_single(value, path)?,
                    path,
                )?,
                path,
            )
        })?;
        let field_description = match (raw_description, raw_description_element) {
            (None, None) => None,
            (value, element) => Some(path.with("description", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_value = path.with("value", |path| {
            let suffix = raw_value
                .suffix
                .ok_or_else(|| path.error(super::super::codec::DecodeErrorKind::MissingProperty))?;
            MeasureReportGroupStratifierStratumComponentValue::from_json_parts(
                suffix,
                raw_value.value,
                raw_value.element,
                path,
            )
        })?;
        Ok(Self {
            id: field_id,
            extension: field_extension,
            modifier_extension: field_modifier_extension,
            link_id: field_link_id,
            code: field_code,
            description: field_description,
            value: field_value,
        })
    }
}

impl serde::Serialize for MeasureReportGroupStratifierStratumComponent {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        let mut map = serde::Serializer::serialize_map(serializer, None)?;
        if let Some(item) = &self.description {
            super::super::codec::element_entry(&mut map, "_description", item)?;
        }
        if let Some(item) = &self.link_id {
            super::super::codec::element_entry(&mut map, "_linkId", item)?;
        }
        match &self.value {
            MeasureReportGroupStratifierStratumComponentValue::Boolean(inner) => {
                super::super::codec::element_entry(&mut map, "_valueBoolean", inner)?;
            }
            MeasureReportGroupStratifierStratumComponentValue::String(inner) => {
                super::super::codec::element_entry(&mut map, "_valueString", inner)?;
            }
            _ => {}
        }
        serde::ser::SerializeMap::serialize_entry(&mut map, "code", &self.code)?;
        if let Some(item) = &self.description {
            super::super::codec::value_entry(&mut map, "description", item)?;
        }
        if !self.extension.is_empty() {
            serde::ser::SerializeMap::serialize_entry(&mut map, "extension", &self.extension)?;
        }
        if let Some(v) = &self.id {
            serde::ser::SerializeMap::serialize_entry(&mut map, "id", v)?;
        }
        if let Some(item) = &self.link_id {
            super::super::codec::value_entry(&mut map, "linkId", item)?;
        }
        if !self.modifier_extension.is_empty() {
            serde::ser::SerializeMap::serialize_entry(
                &mut map,
                "modifierExtension",
                &self.modifier_extension,
            )?;
        }
        match &self.value {
            MeasureReportGroupStratifierStratumComponentValue::Boolean(inner) => {
                super::super::codec::value_entry(&mut map, "valueBoolean", inner)?;
            }
            MeasureReportGroupStratifierStratumComponentValue::CodeableConcept(inner) => {
                serde::ser::SerializeMap::serialize_entry(
                    &mut map,
                    "valueCodeableConcept",
                    inner.as_ref(),
                )?;
            }
            MeasureReportGroupStratifierStratumComponentValue::Period(inner) => {
                serde::ser::SerializeMap::serialize_entry(&mut map, "valuePeriod", inner.as_ref())?;
            }
            MeasureReportGroupStratifierStratumComponentValue::Quantity(inner) => {
                serde::ser::SerializeMap::serialize_entry(
                    &mut map,
                    "valueQuantity",
                    inner.as_ref(),
                )?;
            }
            MeasureReportGroupStratifierStratumComponentValue::Range(inner) => {
                serde::ser::SerializeMap::serialize_entry(&mut map, "valueRange", inner.as_ref())?;
            }
            MeasureReportGroupStratifierStratumComponentValue::Reference(inner) => {
                serde::ser::SerializeMap::serialize_entry(
                    &mut map,
                    "valueReference",
                    inner.as_ref(),
                )?;
            }
            MeasureReportGroupStratifierStratumComponentValue::String(inner) => {
                super::super::codec::value_entry(&mut map, "valueString", inner)?;
            }
        }
        serde::ser::SerializeMap::end(map)
    }
}

impl<'de> serde::Deserialize<'de> for MeasureReportGroupStratifierStratumComponent {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let value = <super::super::codec::Value as serde::Deserialize>::deserialize(deserializer)?;
        let mut path =
            super::super::codec::Path::root("MeasureReportGroupStratifierStratumComponent");
        let object =
            super::super::codec::expect_object(&value, &path).map_err(serde::de::Error::custom)?;
        super::super::codec::Json::from_json(object, &mut path).map_err(serde::de::Error::custom)
    }
}

/// The \`value\[x\]\` choice of
/// \`MeasureReportGroupStratifierStratumComponent\`.
///
/// The stratum component value.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum MeasureReportGroupStratifierStratumComponentValue {
    /// The `CodeableConcept` form.
    CodeableConcept(Box<super::codeable_concept::CodeableConcept>),
    /// The `boolean` form.
    Boolean(super::primitives::Boolean),
    /// The `Period` form.
    Period(Box<super::period::Period>),
    /// The `Quantity` form.
    Quantity(Box<super::quantity::Quantity>),
    /// The `Range` form.
    Range(Box<super::range::Range>),
    /// The `Reference` form.
    Reference(Box<super::reference::Reference>),
    /// The `string` form.
    String(super::primitives::String),
}

impl MeasureReportGroupStratifierStratumComponentValue {
    /// The key suffix, the value part, and the `_name` part of this form.
    ///
    /// # Errors
    ///
    /// Returns [`super::super::codec::EncodeError`] when a held value has no JSON form.
    pub fn to_json_parts(
        &self,
    ) -> Result<
        (
            &'static str,
            Option<super::super::codec::Value>,
            Option<super::super::codec::Value>,
        ),
        super::super::codec::EncodeError,
    > {
        match self {
            Self::CodeableConcept(inner) => Ok((
                "CodeableConcept",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::Boolean(inner) => Ok((
                "Boolean",
                super::super::codec::Primitive::value_json(inner)?,
                super::super::codec::Primitive::element_json(inner)?,
            )),
            Self::Period(inner) => Ok((
                "Period",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::Quantity(inner) => Ok((
                "Quantity",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::Range(inner) => Ok((
                "Range",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::Reference(inner) => Ok((
                "Reference",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::String(inner) => Ok((
                "String",
                super::super::codec::Primitive::value_json(inner)?,
                super::super::codec::Primitive::element_json(inner)?,
            )),
        }
    }

    /// Decodes the form named by `suffix` from its value and `_name` parts.
    ///
    /// # Errors
    ///
    /// Returns [`super::super::codec::DecodeError`] for an unknown suffix or a malformed part.
    pub fn from_json_parts(
        suffix: &str,
        value: Option<&super::super::codec::Value>,
        element: Option<&super::super::codec::Value>,
        path: &mut super::super::codec::Path,
    ) -> Result<Self, super::super::codec::DecodeError> {
        match suffix {
            "CodeableConcept" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::CodeableConcept(Box::new(inner)))
            }
            "Boolean" => Ok(Self::Boolean(
                super::super::codec::Primitive::from_json_parts(value, element, path)?,
            )),
            "Period" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::Period(Box::new(inner)))
            }
            "Quantity" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::Quantity(Box::new(inner)))
            }
            "Range" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::Range(Box::new(inner)))
            }
            "Reference" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::Reference(Box::new(inner)))
            }
            "String" => Ok(Self::String(
                super::super::codec::Primitive::from_json_parts(value, element, path)?,
            )),
            _ => Err(path.error(super::super::codec::DecodeErrorKind::UnknownProperty)),
        }
    }
}

/// The \`measureScore\[x\]\` choice of \`MeasureReportGroupStratifierStratum\`.
///
/// The measure score for this stratum, calculated as appropriate for the
/// measure type and scoring method, and based on only the members of this
/// stratum.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum MeasureReportGroupStratifierStratumMeasureScore {
    /// The `Quantity` form.
    Quantity(Box<super::quantity::Quantity>),
    /// The `dateTime` form.
    DateTime(super::primitives::DateTime),
    /// The `CodeableConcept` form.
    CodeableConcept(Box<super::codeable_concept::CodeableConcept>),
    /// The `Period` form.
    Period(Box<super::period::Period>),
    /// The `Range` form.
    Range(Box<super::range::Range>),
    /// The `Duration` form.
    Duration(Box<super::duration::Duration>),
    /// The `boolean` form.
    Boolean(super::primitives::Boolean),
    /// The `Ratio` form.
    Ratio(Box<super::ratio::Ratio>),
}

impl MeasureReportGroupStratifierStratumMeasureScore {
    /// The key suffix, the value part, and the `_name` part of this form.
    ///
    /// # Errors
    ///
    /// Returns [`super::super::codec::EncodeError`] when a held value has no JSON form.
    pub fn to_json_parts(
        &self,
    ) -> Result<
        (
            &'static str,
            Option<super::super::codec::Value>,
            Option<super::super::codec::Value>,
        ),
        super::super::codec::EncodeError,
    > {
        match self {
            Self::Quantity(inner) => Ok((
                "Quantity",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::DateTime(inner) => Ok((
                "DateTime",
                super::super::codec::Primitive::value_json(inner)?,
                super::super::codec::Primitive::element_json(inner)?,
            )),
            Self::CodeableConcept(inner) => Ok((
                "CodeableConcept",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::Period(inner) => Ok((
                "Period",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::Range(inner) => Ok((
                "Range",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::Duration(inner) => Ok((
                "Duration",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::Boolean(inner) => Ok((
                "Boolean",
                super::super::codec::Primitive::value_json(inner)?,
                super::super::codec::Primitive::element_json(inner)?,
            )),
            Self::Ratio(inner) => Ok((
                "Ratio",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
        }
    }

    /// Decodes the form named by `suffix` from its value and `_name` parts.
    ///
    /// # Errors
    ///
    /// Returns [`super::super::codec::DecodeError`] for an unknown suffix or a malformed part.
    pub fn from_json_parts(
        suffix: &str,
        value: Option<&super::super::codec::Value>,
        element: Option<&super::super::codec::Value>,
        path: &mut super::super::codec::Path,
    ) -> Result<Self, super::super::codec::DecodeError> {
        match suffix {
            "Quantity" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::Quantity(Box::new(inner)))
            }
            "DateTime" => Ok(Self::DateTime(
                super::super::codec::Primitive::from_json_parts(value, element, path)?,
            )),
            "CodeableConcept" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::CodeableConcept(Box::new(inner)))
            }
            "Period" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::Period(Box::new(inner)))
            }
            "Range" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::Range(Box::new(inner)))
            }
            "Duration" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::Duration(Box::new(inner)))
            }
            "Boolean" => Ok(Self::Boolean(
                super::super::codec::Primitive::from_json_parts(value, element, path)?,
            )),
            "Ratio" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::Ratio(Box::new(inner)))
            }
            _ => Err(path.error(super::super::codec::DecodeErrorKind::UnknownProperty)),
        }
    }
}

/// Population results in this stratum
///
/// The populations that make up the stratum, one for each type of population
/// appropriate to the measure. For each stratifier, systems MAY provide
/// population breakdowns in addition to the stratified scores.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct MeasureReportGroupStratifierStratumPopulation {
    /// Unique id for inter-element referencing
    ///
    /// Unique id for the element within a resource (for internal references). This
    /// may be any string value that does not contain spaces.
    pub id: Option<std::string::String>,
    /// Additional content defined by implementations
    ///
    /// May be used to represent additional information that is not part of the
    /// basic definition of the element. To make the use of extensions safe and
    /// managable, there is a strict set of governance applied to the definition and
    /// use of extensions. Though any implementer can define an extension, there is
    /// a set of requirements that SHALL be met as part of the definition of the
    /// extension.
    pub extension: Vec<super::extension::Extension>,
    /// Extensions that cannot be ignored even if unrecognized
    ///
    /// May be used to represent additional information that is not part of the
    /// basic definition of the element and that modifies the understanding of the
    /// element in which it is contained and/or the understanding of the containing
    /// element's descendants. Usually modifier elements provide negation or
    /// qualification. To make the use of extensions safe and managable, there is a
    /// strict set of governance applied to the definition and use of extensions.
    /// Though any implementer can define an extension, there is a set of
    /// requirements that SHALL be met as part of the definition of the extension.
    /// Applications processing a resource are required to check for modifier
    /// extensions. Modifier extensions SHALL NOT change the meaning of any elements
    /// on Resource or DomainResource (including cannot change the meaning of
    /// modifierExtension itself).
    pub modifier_extension: Vec<super::extension::Extension>,
    /// Pointer to specific population from Measure
    ///
    /// The population from the Measure that corresponds to this population in the
    /// MeasureReport resource. This element SHALL be populated with the linkId
    /// corresponding to the population being reported.
    pub link_id: Option<super::primitives::String>,
    /// initial-population | numerator | numerator-exclusion | denominator |
    /// denominator-exclusion | denominator-exception | measure-population |
    /// measure-population-exclusion | measure-observation
    ///
    /// The type of the population. This element SHALL be populated with at least
    /// the codings in the code element of the corresponding population being
    /// reported.
    pub code: Option<super::codeable_concept::CodeableConcept>,
    /// Size of the population
    ///
    /// The number of members of the population in this stratum.
    pub count: Option<super::primitives::Integer>,
    /// Size of the population as a quantity
    ///
    /// The number of members of the population in this stratum, specified as a
    /// quantity to support identifying units, as well as to support some composite
    /// measure calculation use cases where the resulting count of the population is
    /// a decimal value.
    pub count_quantity: Option<super::quantity::Quantity>,
    /// For subject-list reports, the subject results in this population
    ///
    /// This element refers to a List of individual level MeasureReport resources,
    /// one for each subject in this population in this stratum.
    pub subject_results: Option<super::reference::Reference>,
    /// For subject-list reports, a subject result in this population
    ///
    /// A reference to an individual level MeasureReport resource for a member of
    /// the population.
    pub subject_report: Vec<super::reference::Reference>,
    /// What individual(s) in the population
    ///
    /// Optional Group identifying the individuals that make up the population.
    pub subjects: Option<super::reference::Reference>,
}

impl super::super::codec::Json for MeasureReportGroupStratifierStratumPopulation {
    fn to_json(&self) -> Result<super::super::codec::Object, super::super::codec::EncodeError> {
        let mut object = super::super::codec::Object::new();
        if let Some(v) = &self.id {
            object.insert(
                std::string::String::from("id"),
                super::super::codec::Value::String(v.clone()),
            );
        }
        if !self.extension.is_empty() {
            let mut items = Vec::with_capacity(self.extension.len());
            for item in &self.extension {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("extension"),
                super::super::codec::Value::Array(items),
            );
        }
        if !self.modifier_extension.is_empty() {
            let mut items = Vec::with_capacity(self.modifier_extension.len());
            for item in &self.modifier_extension {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("modifierExtension"),
                super::super::codec::Value::Array(items),
            );
        }
        if let Some(item) = &self.link_id {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("linkId"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_linkId"), e);
            }
        }
        if let Some(item) = &self.code {
            object.insert(
                std::string::String::from("code"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if let Some(item) = &self.count {
            if let Some(v) = super::super::codec::Primitive::value_json(item)? {
                object.insert(std::string::String::from("count"), v);
            }
            if let Some(e) = super::super::codec::Primitive::element_json(item)? {
                object.insert(std::string::String::from("_count"), e);
            }
        }
        if let Some(item) = &self.count_quantity {
            object.insert(
                std::string::String::from("countQuantity"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if let Some(item) = &self.subject_results {
            object.insert(
                std::string::String::from("subjectResults"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        if !self.subject_report.is_empty() {
            let mut items = Vec::with_capacity(self.subject_report.len());
            for item in &self.subject_report {
                items.push(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(item)?,
                ));
            }
            object.insert(
                std::string::String::from("subjectReport"),
                super::super::codec::Value::Array(items),
            );
        }
        if let Some(item) = &self.subjects {
            object.insert(
                std::string::String::from("subjects"),
                super::super::codec::Value::Object(super::super::codec::Json::to_json(item)?),
            );
        }
        Ok(object)
    }

    fn from_json(
        object: &super::super::codec::Object,
        path: &mut super::super::codec::Path,
    ) -> Result<Self, super::super::codec::DecodeError> {
        let mut raw_id: Option<&super::super::codec::Value> = None;
        let mut raw_extension: Option<&super::super::codec::Value> = None;
        let mut raw_modifier_extension: Option<&super::super::codec::Value> = None;
        let mut raw_link_id: Option<&super::super::codec::Value> = None;
        let mut raw_link_id_element: Option<&super::super::codec::Value> = None;
        let mut raw_code: Option<&super::super::codec::Value> = None;
        let mut raw_count: Option<&super::super::codec::Value> = None;
        let mut raw_count_element: Option<&super::super::codec::Value> = None;
        let mut raw_count_quantity: Option<&super::super::codec::Value> = None;
        let mut raw_subject_results: Option<&super::super::codec::Value> = None;
        let mut raw_subject_report: Option<&super::super::codec::Value> = None;
        let mut raw_subjects: Option<&super::super::codec::Value> = None;
        for (key, value) in object {
            match key.as_str() {
                "id" => {
                    raw_id = Some(value);
                }
                "extension" => {
                    raw_extension = Some(value);
                }
                "modifierExtension" => {
                    raw_modifier_extension = Some(value);
                }
                "linkId" => {
                    raw_link_id = Some(value);
                }
                "_linkId" => {
                    raw_link_id_element = Some(value);
                }
                "code" => {
                    raw_code = Some(value);
                }
                "count" => {
                    raw_count = Some(value);
                }
                "_count" => {
                    raw_count_element = Some(value);
                }
                "countQuantity" => {
                    raw_count_quantity = Some(value);
                }
                "subjectResults" => {
                    raw_subject_results = Some(value);
                }
                "subjectReport" => {
                    raw_subject_report = Some(value);
                }
                "subjects" => {
                    raw_subjects = Some(value);
                }
                other => {
                    return path.with(other, |path| {
                        Err(path.error(super::super::codec::DecodeErrorKind::UnknownProperty))
                    });
                }
            }
        }
        let field_id = raw_id
            .map(|value| {
                path.with("id", |path| {
                    let value = super::super::codec::expect_single(value, path)?;
                    super::primitives::checked_string(
                        super::super::codec::expect_string(value, path)?,
                        path,
                    )
                })
            })
            .transpose()?;
        let mut field_extension = Vec::new();
        if let Some(raw) = raw_extension {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_extension.push(path.with_index("extension", index, |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(value, path)?,
                        path,
                    )
                })?);
            }
        }
        let mut field_modifier_extension = Vec::new();
        if let Some(raw) = raw_modifier_extension {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_modifier_extension.push(path.with_index(
                    "modifierExtension",
                    index,
                    |path| {
                        super::super::codec::Json::from_json(
                            super::super::codec::expect_object(value, path)?,
                            path,
                        )
                    },
                )?);
            }
        }
        let field_link_id = match (raw_link_id, raw_link_id_element) {
            (None, None) => None,
            (value, element) => Some(path.with("linkId", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_code = raw_code
            .map(|value| {
                path.with("code", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let field_count = match (raw_count, raw_count_element) {
            (None, None) => None,
            (value, element) => Some(path.with("count", |path| {
                super::super::codec::Primitive::from_json_parts(value, element, path)
            })?),
        };
        let field_count_quantity = raw_count_quantity
            .map(|value| {
                path.with("countQuantity", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let field_subject_results = raw_subject_results
            .map(|value| {
                path.with("subjectResults", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        let mut field_subject_report = Vec::new();
        if let Some(raw) = raw_subject_report {
            for (index, value) in super::super::codec::expect_array(raw, path)?
                .iter()
                .enumerate()
            {
                field_subject_report.push(path.with_index("subjectReport", index, |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(value, path)?,
                        path,
                    )
                })?);
            }
        }
        let field_subjects = raw_subjects
            .map(|value| {
                path.with("subjects", |path| {
                    super::super::codec::Json::from_json(
                        super::super::codec::expect_object(
                            super::super::codec::expect_single(value, path)?,
                            path,
                        )?,
                        path,
                    )
                })
            })
            .transpose()?;
        Ok(Self {
            id: field_id,
            extension: field_extension,
            modifier_extension: field_modifier_extension,
            link_id: field_link_id,
            code: field_code,
            count: field_count,
            count_quantity: field_count_quantity,
            subject_results: field_subject_results,
            subject_report: field_subject_report,
            subjects: field_subjects,
        })
    }
}

impl serde::Serialize for MeasureReportGroupStratifierStratumPopulation {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        let mut map = serde::Serializer::serialize_map(serializer, None)?;
        if let Some(item) = &self.count {
            super::super::codec::element_entry(&mut map, "_count", item)?;
        }
        if let Some(item) = &self.link_id {
            super::super::codec::element_entry(&mut map, "_linkId", item)?;
        }
        if let Some(item) = &self.code {
            serde::ser::SerializeMap::serialize_entry(&mut map, "code", item)?;
        }
        if let Some(item) = &self.count {
            super::super::codec::value_entry(&mut map, "count", item)?;
        }
        if let Some(item) = &self.count_quantity {
            serde::ser::SerializeMap::serialize_entry(&mut map, "countQuantity", item)?;
        }
        if !self.extension.is_empty() {
            serde::ser::SerializeMap::serialize_entry(&mut map, "extension", &self.extension)?;
        }
        if let Some(v) = &self.id {
            serde::ser::SerializeMap::serialize_entry(&mut map, "id", v)?;
        }
        if let Some(item) = &self.link_id {
            super::super::codec::value_entry(&mut map, "linkId", item)?;
        }
        if !self.modifier_extension.is_empty() {
            serde::ser::SerializeMap::serialize_entry(
                &mut map,
                "modifierExtension",
                &self.modifier_extension,
            )?;
        }
        if !self.subject_report.is_empty() {
            serde::ser::SerializeMap::serialize_entry(
                &mut map,
                "subjectReport",
                &self.subject_report,
            )?;
        }
        if let Some(item) = &self.subject_results {
            serde::ser::SerializeMap::serialize_entry(&mut map, "subjectResults", item)?;
        }
        if let Some(item) = &self.subjects {
            serde::ser::SerializeMap::serialize_entry(&mut map, "subjects", item)?;
        }
        serde::ser::SerializeMap::end(map)
    }
}

impl<'de> serde::Deserialize<'de> for MeasureReportGroupStratifierStratumPopulation {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let value = <super::super::codec::Value as serde::Deserialize>::deserialize(deserializer)?;
        let mut path =
            super::super::codec::Path::root("MeasureReportGroupStratifierStratumPopulation");
        let object =
            super::super::codec::expect_object(&value, &path).map_err(serde::de::Error::custom)?;
        super::super::codec::Json::from_json(object, &mut path).map_err(serde::de::Error::custom)
    }
}

/// The \`value\[x\]\` choice of \`MeasureReportGroupStratifierStratum\`.
///
/// The value for this stratum, expressed as a CodeableConcept. When defining
/// stratifiers on complex values, the value must be rendered such that the
/// value for each stratum within the stratifier is unique.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum MeasureReportGroupStratifierStratumValue {
    /// The `CodeableConcept` form.
    CodeableConcept(Box<super::codeable_concept::CodeableConcept>),
    /// The `boolean` form.
    Boolean(super::primitives::Boolean),
    /// The `Period` form.
    Period(Box<super::period::Period>),
    /// The `Quantity` form.
    Quantity(Box<super::quantity::Quantity>),
    /// The `Range` form.
    Range(Box<super::range::Range>),
    /// The `Reference` form.
    Reference(Box<super::reference::Reference>),
    /// The `string` form.
    String(super::primitives::String),
}

impl MeasureReportGroupStratifierStratumValue {
    /// The key suffix, the value part, and the `_name` part of this form.
    ///
    /// # Errors
    ///
    /// Returns [`super::super::codec::EncodeError`] when a held value has no JSON form.
    pub fn to_json_parts(
        &self,
    ) -> Result<
        (
            &'static str,
            Option<super::super::codec::Value>,
            Option<super::super::codec::Value>,
        ),
        super::super::codec::EncodeError,
    > {
        match self {
            Self::CodeableConcept(inner) => Ok((
                "CodeableConcept",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::Boolean(inner) => Ok((
                "Boolean",
                super::super::codec::Primitive::value_json(inner)?,
                super::super::codec::Primitive::element_json(inner)?,
            )),
            Self::Period(inner) => Ok((
                "Period",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::Quantity(inner) => Ok((
                "Quantity",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::Range(inner) => Ok((
                "Range",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::Reference(inner) => Ok((
                "Reference",
                Some(super::super::codec::Value::Object(
                    super::super::codec::Json::to_json(inner.as_ref())?,
                )),
                None,
            )),
            Self::String(inner) => Ok((
                "String",
                super::super::codec::Primitive::value_json(inner)?,
                super::super::codec::Primitive::element_json(inner)?,
            )),
        }
    }

    /// Decodes the form named by `suffix` from its value and `_name` parts.
    ///
    /// # Errors
    ///
    /// Returns [`super::super::codec::DecodeError`] for an unknown suffix or a malformed part.
    pub fn from_json_parts(
        suffix: &str,
        value: Option<&super::super::codec::Value>,
        element: Option<&super::super::codec::Value>,
        path: &mut super::super::codec::Path,
    ) -> Result<Self, super::super::codec::DecodeError> {
        match suffix {
            "CodeableConcept" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::CodeableConcept(Box::new(inner)))
            }
            "Boolean" => Ok(Self::Boolean(
                super::super::codec::Primitive::from_json_parts(value, element, path)?,
            )),
            "Period" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::Period(Box::new(inner)))
            }
            "Quantity" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::Quantity(Box::new(inner)))
            }
            "Range" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::Range(Box::new(inner)))
            }
            "Reference" => {
                if element.is_some() {
                    return Err(path.error(super::super::codec::DecodeErrorKind::WrongType {
                        expected: "no underscore form for a complex type",
                    }));
                }
                let value = value.ok_or_else(|| {
                    path.error(super::super::codec::DecodeErrorKind::MissingProperty)
                })?;
                let inner = super::super::codec::Json::from_json(
                    super::super::codec::expect_object(
                        super::super::codec::expect_single(value, path)?,
                        path,
                    )?,
                    path,
                )?;
                Ok(Self::Reference(Box::new(inner)))
            }
            "String" => Ok(Self::String(
                super::super::codec::Primitive::from_json_parts(value, element, path)?,
            )),
            _ => Err(path.error(super::super::codec::DecodeErrorKind::UnknownProperty)),
        }
    }
}
