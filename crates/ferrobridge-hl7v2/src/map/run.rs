...
impl<'a> Run<'a> {
    ...
    fn family(&self, base: &Base, reference: &RowRef) -> Option<usize> {
    ...
    fn sibling(&mut self, anchor: usize, label: u32) -> usize {
...
mod tests {
    ...
    fn written(resource: &super::Resource, path: &str) -> Option<String> {
    ...
    fn chain(run: &Run<'_>) -> Vec<(String, String)> {
        let locations: BTreeMap<&str, &super::Resource> = run
            .resources
            .iter()
    ...
    fn locations<'r>(run: &'r Run<'_>) -> Vec<&'r super::Resource> {
        run.resources
            .iter()
            .filter(|resource| resource.type_name == "Location" && !resource.omitted)
            .collect()
