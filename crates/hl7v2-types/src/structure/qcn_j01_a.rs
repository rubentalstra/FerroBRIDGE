// @generated by fhir-codegen from HL7/v2ig 3adcdbfff654ccbff5cd33aa34bd909a087e8e19 input/sourceOfTruth. DO NOT EDIT.
// Change the emitter (tools/fhir-codegen) and regenerate.
// SPDX-FileCopyrightText: Vernum Projecten B.V.
// SPDX-License-Identifier: Apache-2.0
//! The `QCN_J01-A` message structure.

use crate::model::{Cardinality, Max, Node, SegmentRef, SegmentStatus, Structure};
use crate::segment;

/// The top-level nodes of [`QCN_J01_A`], one `static` so a structure with the same tree links to it.
pub static QCN_J01_A_NODES: [Node; 4] = [
    Node::Segment(SegmentRef {
        id: "QCN_J01-A.1-MSH",
        position: 1,
        segment: &segment::msh::MSH,
        cardinality: Cardinality {
            min: 1,
            max: Max::Bounded(1),
        },
        status: Some(SegmentStatus::A),
    }),
    Node::Segment(SegmentRef {
        id: "QCN_J01-A.2-SFT",
        position: 2,
        segment: &segment::sft::SFT,
        cardinality: Cardinality {
            min: 0,
            max: Max::Unbounded,
        },
        status: Some(SegmentStatus::A),
    }),
    Node::Segment(SegmentRef {
        id: "QCN_J01-A.3-UAC",
        position: 3,
        segment: &segment::uac::UAC,
        cardinality: Cardinality {
            min: 0,
            max: Max::Bounded(1),
        },
        status: Some(SegmentStatus::A),
    }),
    Node::Segment(SegmentRef {
        id: "QCN_J01-A.4-QID",
        position: 4,
        segment: &segment::qid::QID,
        cardinality: Cardinality {
            min: 1,
            max: Max::Bounded(1),
        },
        status: Some(SegmentStatus::A),
    }),
];

/// The `QCN_J01-A` message structure definition, `http://hl7.org/v2/StructureDefinition/QCN_J01-A`.
pub static QCN_J01_A: Structure = Structure {
    id: "QCN_J01-A",
    url: Some("http://hl7.org/v2/StructureDefinition/QCN_J01-A"),
    version: "2.9.1",
    withdrawn_as_of: None,
    nodes: &QCN_J01_A_NODES,
};
