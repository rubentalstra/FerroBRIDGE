// @generated by fhir-codegen from usnistgov/igamt-hl7Tools-service 83322dffdac18cb129a1bd80149f5439cf5686fd src/main/resources/hl7db. DO NOT EDIT.
// Change the emitter (tools/fhir-codegen) and regenerate.
// SPDX-FileCopyrightText: Vernum Projecten B.V.
// SPDX-License-Identifier: Apache-2.0
//! The `ARQ` segment: ARQ - appointment request segment.

use crate::model::{Cardinality, DataTypeRef, Field, Length, Max, Optionality, Segment, Table};

/// The `ARQ` segment of the 2.3.1 tables.
pub static ARQ: Segment = Segment {
    id: "ARQ",
    url: None,
    name: "ARQ - appointment request segment",
    fields: &[
        Field {
            id: "ARQ.1",
            position: 1,
            name: "Placer Appointment ID",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::EI)),
            cardinality: Cardinality {
                min: 1,
                max: Max::Bounded(1),
            },
            optionality: Optionality::R,
            length: Some(Length {
                min: 0,
                max: Some(75),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.2",
            position: 2,
            name: "Filler Appointment ID",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::EI)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::C,
            length: Some(Length {
                min: 0,
                max: Some(75),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.3",
            position: 3,
            name: "Occurrence Number",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::NM)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::C,
            length: Some(Length {
                min: 0,
                max: Some(5),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.4",
            position: 4,
            name: "Placer Group Number",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::EI)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(22),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.5",
            position: 5,
            name: "Schedule ID",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::CE)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(200),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.6",
            position: 6,
            name: "Request Event Reason",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::CE)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(200),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.7",
            position: 7,
            name: "Appointment Reason",
            data_type: Some(DataTypeRef::Legacy(
                &crate::legacy::v2_3_1::data_type::CE_0276,
            )),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(200),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0276",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0276",
            }),
            standards_status: None,
        },
        Field {
            id: "ARQ.8",
            position: 8,
            name: "Appointment Type",
            data_type: Some(DataTypeRef::Legacy(
                &crate::legacy::v2_3_1::data_type::CE_0277,
            )),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(200),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0277",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0277",
            }),
            standards_status: None,
        },
        Field {
            id: "ARQ.9",
            position: 9,
            name: "Appointment Duration",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::NM)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(20),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.10",
            position: 10,
            name: "Appointment Duration Units",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::CE)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(200),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.11",
            position: 11,
            name: "Requested Start Date/Time Range",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::DR)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(53),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.12",
            position: 12,
            name: "Priority-ARQ",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::ST)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(5),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.13",
            position: 13,
            name: "Repeating Interval",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::RI)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(100),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.14",
            position: 14,
            name: "Repeating Interval Duration",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::ST)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(5),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.15",
            position: 15,
            name: "Placer Contact Person",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::XCN)),
            cardinality: Cardinality {
                min: 1,
                max: Max::Unbounded,
            },
            optionality: Optionality::R,
            length: Some(Length {
                min: 0,
                max: Some(48),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.16",
            position: 16,
            name: "Placer Contact Phone Number",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::XTN)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(40),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.17",
            position: 17,
            name: "Placer Contact Address",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::XAD)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(106),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.18",
            position: 18,
            name: "Placer Contact Location",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::PL)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(80),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.19",
            position: 19,
            name: "Entered by Person",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::XCN)),
            cardinality: Cardinality {
                min: 1,
                max: Max::Unbounded,
            },
            optionality: Optionality::R,
            length: Some(Length {
                min: 0,
                max: Some(48),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.20",
            position: 20,
            name: "Entered by Phone Number",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::XTN)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(40),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.21",
            position: 21,
            name: "Entered by Location",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::PL)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(80),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.22",
            position: 22,
            name: "Parent Placer Appointment ID",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::EI)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(75),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "ARQ.23",
            position: 23,
            name: "Parent Filler Appointment ID",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3_1::data_type::EI)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(75),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
    ],
};
