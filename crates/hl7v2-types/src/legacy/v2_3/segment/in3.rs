// @generated by fhir-codegen from usnistgov/igamt-hl7Tools-service 83322dffdac18cb129a1bd80149f5439cf5686fd src/main/resources/hl7db. DO NOT EDIT.
// Change the emitter (tools/fhir-codegen) and regenerate.
// SPDX-FileCopyrightText: Vernum Projecten B.V.
// SPDX-License-Identifier: Apache-2.0
//! The `IN3` segment: Insurance additional info - certification.

use crate::model::{Cardinality, DataTypeRef, Field, Length, Max, Optionality, Segment, Table};

/// The `IN3` segment of the 2.3 tables.
pub static IN3: Segment = Segment {
    id: "IN3",
    url: None,
    name: "Insurance additional info - certification",
    fields: &[
        Field {
            id: "IN3.1",
            position: 1,
            name: "Set ID - Insurance Certification",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::SI)),
            cardinality: Cardinality {
                min: 1,
                max: Max::Bounded(1),
            },
            optionality: Optionality::R,
            length: Some(Length {
                min: 0,
                max: Some(4),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "IN3.2",
            position: 2,
            name: "Certification Number",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::CX)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(59),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "IN3.3",
            position: 3,
            name: "Certified By",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::XCN)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(60),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "IN3.4",
            position: 4,
            name: "Certification Required",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::ID)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(1),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0136",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0136",
            }),
            standards_status: None,
        },
        Field {
            id: "IN3.5",
            position: 5,
            name: "Penalty",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::CM_PEN)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(10),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0148",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0148",
            }),
            standards_status: None,
        },
        Field {
            id: "IN3.6",
            position: 6,
            name: "Certification Date/Time",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::TS)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(26),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "IN3.7",
            position: 7,
            name: "Certification Modify Date/Time",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::TS)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::Na,
            length: Some(Length {
                min: 0,
                max: Some(26),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "IN3.8",
            position: 8,
            name: "Operator",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::XCN)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::Na,
            length: Some(Length {
                min: 0,
                max: Some(60),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "IN3.9",
            position: 9,
            name: "Certification Begin Date",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::DT)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::Na,
            length: Some(Length {
                min: 0,
                max: Some(8),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "IN3.10",
            position: 10,
            name: "Certification End Date",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::DT)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::Na,
            length: Some(Length {
                min: 0,
                max: Some(8),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "IN3.11",
            position: 11,
            name: "Days",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::CM_DTN)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::Na,
            length: Some(Length {
                min: 0,
                max: Some(3),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0149",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0149",
            }),
            standards_status: None,
        },
        Field {
            id: "IN3.12",
            position: 12,
            name: "Non-Concur Code/Description",
            data_type: Some(DataTypeRef::Legacy(
                &crate::legacy::v2_3::data_type::CE_0233,
            )),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::Na,
            length: Some(Length {
                min: 0,
                max: Some(60),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0233",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0233",
            }),
            standards_status: None,
        },
        Field {
            id: "IN3.13",
            position: 13,
            name: "Non-Concur Effective Date/Time",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::TS)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::Na,
            length: Some(Length {
                min: 0,
                max: Some(26),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "IN3.14",
            position: 14,
            name: "Physician Reviewer",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::XCN)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::Na,
            length: Some(Length {
                min: 0,
                max: Some(60),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "IN3.15",
            position: 15,
            name: "Certification Contact",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::ST)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::Na,
            length: Some(Length {
                min: 0,
                max: Some(48),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "IN3.16",
            position: 16,
            name: "Certification Contact Phone Number",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::XTN)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::Na,
            length: Some(Length {
                min: 0,
                max: Some(40),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "IN3.17",
            position: 17,
            name: "Appeal Reason",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::CE)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::Na,
            length: Some(Length {
                min: 0,
                max: Some(60),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "IN3.18",
            position: 18,
            name: "Certification Agency",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::CE)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::Na,
            length: Some(Length {
                min: 0,
                max: Some(60),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "IN3.19",
            position: 19,
            name: "Certification Agency Phone Number",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::XTN)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::Na,
            length: Some(Length {
                min: 0,
                max: Some(40),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "IN3.20",
            position: 20,
            name: "Pre-Certification required/Window",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::CM_PCF)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::Na,
            length: Some(Length {
                min: 0,
                max: Some(40),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "IN3.21",
            position: 21,
            name: "Case Manager",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::ST)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::Na,
            length: Some(Length {
                min: 0,
                max: Some(48),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "IN3.22",
            position: 22,
            name: "Second Opinion Date",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::DT)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::Na,
            length: Some(Length {
                min: 0,
                max: Some(8),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "IN3.23",
            position: 23,
            name: "Second Opinion Status",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::IS)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::Na,
            length: Some(Length {
                min: 0,
                max: Some(1),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0151",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0151",
            }),
            standards_status: None,
        },
        Field {
            id: "IN3.24",
            position: 24,
            name: "Second Opinion Documentation Received",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::IS)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::Na,
            length: Some(Length {
                min: 0,
                max: Some(1),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0152",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0152",
            }),
            standards_status: None,
        },
        Field {
            id: "IN3.25",
            position: 25,
            name: "Second Opinion Physician",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_3::data_type::XCN)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::Na,
            length: Some(Length {
                min: 0,
                max: Some(60),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
    ],
};
