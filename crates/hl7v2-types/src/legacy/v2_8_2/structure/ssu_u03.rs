// @generated by fhir-codegen from usnistgov/igamt-hl7Tools-service 83322dffdac18cb129a1bd80149f5439cf5686fd src/main/resources/hl7db. DO NOT EDIT.
// Change the emitter (tools/fhir-codegen) and regenerate.
// SPDX-FileCopyrightText: Vernum Projecten B.V.
// SPDX-License-Identifier: Apache-2.0
//! The `SSU_U03` message structure of the 2.8.2 tables.

use crate::model::{Cardinality, Group, GroupKind, Max, Node, SegmentRef, Structure};

/// The top-level nodes of [`SSU_U03`], one `static` so a structure with the same tree links to it.
pub static SSU_U03_NODES: [Node; 5] = [
    Node::Segment(SegmentRef {
        id: "SSU_U03.1-MSH",
        position: 1,
        segment: &crate::legacy::v2_7_1::segment::msh::MSH,
        cardinality: Cardinality {
            min: 1,
            max: Max::Bounded(1),
        },
        status: None,
    }),
    Node::Segment(SegmentRef {
        id: "SSU_U03.2-SFT",
        position: 2,
        segment: &crate::segment::sft::SFT,
        cardinality: Cardinality {
            min: 0,
            max: Max::Unbounded,
        },
        status: None,
    }),
    Node::Segment(SegmentRef {
        id: "SSU_U03.3-UAC",
        position: 3,
        segment: &crate::segment::uac::UAC,
        cardinality: Cardinality {
            min: 0,
            max: Max::Bounded(1),
        },
        status: None,
    }),
    Node::Segment(SegmentRef {
        id: "SSU_U03.4-EQU",
        position: 4,
        segment: &crate::legacy::v2_8::segment::equ::EQU,
        cardinality: Cardinality {
            min: 1,
            max: Max::Bounded(1),
        },
        status: None,
    }),
    Node::Group(Group {
        id: "SSU_U03.5-SPECIMEN_CONTAINER",
        position: 5,
        name: "SPECIMEN_CONTAINER",
        cardinality: Cardinality {
            min: 1,
            max: Max::Unbounded,
        },
        kind: GroupKind::Sequence,
        children: &[
            Node::Segment(SegmentRef {
                id: "SSU_U03.5-SPECIMEN_CONTAINER.1-SAC",
                position: 1,
                segment: &crate::legacy::v2_7_1::segment::sac::SAC,
                cardinality: Cardinality {
                    min: 1,
                    max: Max::Bounded(1),
                },
                status: None,
            }),
            Node::Segment(SegmentRef {
                id: "SSU_U03.5-SPECIMEN_CONTAINER.2-OBX",
                position: 2,
                segment: &crate::legacy::v2_8_2::segment::obx::OBX,
                cardinality: Cardinality {
                    min: 0,
                    max: Max::Unbounded,
                },
                status: None,
            }),
            Node::Segment(SegmentRef {
                id: "SSU_U03.5-SPECIMEN_CONTAINER.3-NTE",
                position: 3,
                segment: &crate::legacy::v2_7_1::segment::nte::NTE,
                cardinality: Cardinality {
                    min: 0,
                    max: Max::Unbounded,
                },
                status: None,
            }),
            Node::Group(Group {
                id: "SSU_U03.5-SPECIMEN_CONTAINER.4-SPECIMEN",
                position: 4,
                name: "SPECIMEN",
                cardinality: Cardinality {
                    min: 0,
                    max: Max::Unbounded,
                },
                kind: GroupKind::Sequence,
                children: &[
                    Node::Segment(SegmentRef {
                        id: "SSU_U03.5-SPECIMEN_CONTAINER.4-SPECIMEN.1-SPM",
                        position: 1,
                        segment: &crate::legacy::v2_8_2::segment::spm::SPM,
                        cardinality: Cardinality {
                            min: 1,
                            max: Max::Bounded(1),
                        },
                        status: None,
                    }),
                    Node::Segment(SegmentRef {
                        id: "SSU_U03.5-SPECIMEN_CONTAINER.4-SPECIMEN.2-OBX",
                        position: 2,
                        segment: &crate::legacy::v2_8_2::segment::obx::OBX,
                        cardinality: Cardinality {
                            min: 0,
                            max: Max::Unbounded,
                        },
                        status: None,
                    }),
                ],
            }),
        ],
    }),
];

/// The `SSU_U03` message structure of the 2.8.2 tables.
pub static SSU_U03: Structure = Structure {
    id: "SSU_U03",
    url: None,
    version: "2.8.2",
    withdrawn_as_of: None,
    nodes: &SSU_U03_NODES,
};
