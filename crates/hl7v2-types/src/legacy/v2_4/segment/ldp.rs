// @generated by fhir-codegen from usnistgov/igamt-hl7Tools-service 83322dffdac18cb129a1bd80149f5439cf5686fd src/main/resources/hl7db. DO NOT EDIT.
// Change the emitter (tools/fhir-codegen) and regenerate.
// SPDX-FileCopyrightText: Vernum Projecten B.V.
// SPDX-License-Identifier: Apache-2.0
//! The `LDP` segment: Location Department.

use crate::model::{Cardinality, DataTypeRef, Field, Length, Max, Optionality, Segment, Table};

/// The `LDP` segment of the 2.4 tables.
pub static LDP: Segment = Segment {
    id: "LDP",
    url: None,
    name: "Location Department",
    fields: &[
        Field {
            id: "LDP.1",
            position: 1,
            name: "Primary Key Value - LDP",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::PL)),
            cardinality: Cardinality {
                min: 1,
                max: Max::Bounded(1),
            },
            optionality: Optionality::R,
            length: Some(Length {
                min: 0,
                max: Some(200),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "LDP.2",
            position: 2,
            name: "Location Department",
            data_type: Some(DataTypeRef::Legacy(
                &crate::legacy::v2_4::data_type::CE_0264,
            )),
            cardinality: Cardinality {
                min: 1,
                max: Max::Bounded(1),
            },
            optionality: Optionality::R,
            length: Some(Length {
                min: 0,
                max: Some(250),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0264",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0264",
            }),
            standards_status: None,
        },
        Field {
            id: "LDP.3",
            position: 3,
            name: "Location Service",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::IS)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(3),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0069",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0069",
            }),
            standards_status: None,
        },
        Field {
            id: "LDP.4",
            position: 4,
            name: "Specialty Type",
            data_type: Some(DataTypeRef::Legacy(
                &crate::legacy::v2_4::data_type::CE_0265,
            )),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(250),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0265",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0265",
            }),
            standards_status: None,
        },
        Field {
            id: "LDP.5",
            position: 5,
            name: "Valid Patient Classes",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::IS)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(1),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0004",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0004",
            }),
            standards_status: None,
        },
        Field {
            id: "LDP.6",
            position: 6,
            name: "Active/Inactive Flag",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::ID)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(1),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0183",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0183",
            }),
            standards_status: None,
        },
        Field {
            id: "LDP.7",
            position: 7,
            name: "Activation Date  LDP",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::TS)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(26),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "LDP.8",
            position: 8,
            name: "Inactivation Date - LDP",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::TS)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(26),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "LDP.9",
            position: 9,
            name: "Inactivated Reason",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::ST)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(80),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "LDP.10",
            position: 10,
            name: "Visiting Hours",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::VH)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(80),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0267",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0267",
            }),
            standards_status: None,
        },
        Field {
            id: "LDP.11",
            position: 11,
            name: "Contact Phone",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::XTN)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(250),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "LDP.12",
            position: 12,
            name: "Location Cost Center",
            data_type: Some(DataTypeRef::Legacy(
                &crate::legacy::v2_4::data_type::CE_0462,
            )),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(250),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0462",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0462",
            }),
            standards_status: None,
        },
    ],
};
