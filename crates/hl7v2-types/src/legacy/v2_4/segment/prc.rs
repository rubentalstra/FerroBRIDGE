// @generated by fhir-codegen from usnistgov/igamt-hl7Tools-service 83322dffdac18cb129a1bd80149f5439cf5686fd src/main/resources/hl7db. DO NOT EDIT.
// Change the emitter (tools/fhir-codegen) and regenerate.
// SPDX-FileCopyrightText: Vernum Projecten B.V.
// SPDX-License-Identifier: Apache-2.0
//! The `PRC` segment: Pricing.

use crate::model::{Cardinality, DataTypeRef, Field, Length, Max, Optionality, Segment, Table};

/// The `PRC` segment of the 2.4 tables.
pub static PRC: Segment = Segment {
    id: "PRC",
    url: None,
    name: "Pricing",
    fields: &[
        Field {
            id: "PRC.1",
            position: 1,
            name: "Primary Key Value - PRC",
            data_type: Some(DataTypeRef::Legacy(
                &crate::legacy::v2_4::data_type::CE_0132,
            )),
            cardinality: Cardinality {
                min: 1,
                max: Max::Bounded(1),
            },
            optionality: Optionality::R,
            length: Some(Length {
                min: 0,
                max: Some(250),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0132",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0132",
            }),
            standards_status: None,
        },
        Field {
            id: "PRC.2",
            position: 2,
            name: "Facility ID - PRC",
            data_type: Some(DataTypeRef::Legacy(
                &crate::legacy::v2_4::data_type::CE_0464,
            )),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(250),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0464",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0464",
            }),
            standards_status: None,
        },
        Field {
            id: "PRC.3",
            position: 3,
            name: "Department",
            data_type: Some(DataTypeRef::Legacy(
                &crate::legacy::v2_4::data_type::CE_0184,
            )),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(250),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0184",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0184",
            }),
            standards_status: None,
        },
        Field {
            id: "PRC.4",
            position: 4,
            name: "Valid Patient Classes",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::IS)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(1),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0004",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0004",
            }),
            standards_status: None,
        },
        Field {
            id: "PRC.5",
            position: 5,
            name: "Price",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::CP)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::C,
            length: Some(Length {
                min: 0,
                max: Some(12),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "PRC.6",
            position: 6,
            name: "Formula",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::ST)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(200),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "PRC.7",
            position: 7,
            name: "Minimum Quantity",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::NM)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(4),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "PRC.8",
            position: 8,
            name: "Maximum Quantity",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::NM)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(4),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "PRC.9",
            position: 9,
            name: "Minimum Price",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::MO)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(12),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "PRC.10",
            position: 10,
            name: "Maximum Price",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::MO)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(12),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "PRC.11",
            position: 11,
            name: "Effective Start Date",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::TS)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(26),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "PRC.12",
            position: 12,
            name: "Effective End Date",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::TS)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(26),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "PRC.13",
            position: 13,
            name: "Price Override Flag",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::IS)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(1),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0268",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0268",
            }),
            standards_status: None,
        },
        Field {
            id: "PRC.14",
            position: 14,
            name: "Billing Category",
            data_type: Some(DataTypeRef::Legacy(
                &crate::legacy::v2_4::data_type::CE_0293,
            )),
            cardinality: Cardinality {
                min: 0,
                max: Max::Unbounded,
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(250),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0293",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0293",
            }),
            standards_status: None,
        },
        Field {
            id: "PRC.15",
            position: 15,
            name: "Chargeable Flag",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::ID)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(1),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0136",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0136",
            }),
            standards_status: None,
        },
        Field {
            id: "PRC.16",
            position: 16,
            name: "Active/Inactive Flag",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::ID)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(1),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0183",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0183",
            }),
            standards_status: None,
        },
        Field {
            id: "PRC.17",
            position: 17,
            name: "Cost",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::MO)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(12),
            }),
            conformance_length: None,
            table: None,
            standards_status: None,
        },
        Field {
            id: "PRC.18",
            position: 18,
            name: "Charge On Indicator",
            data_type: Some(DataTypeRef::Legacy(&crate::legacy::v2_4::data_type::IS)),
            cardinality: Cardinality {
                min: 0,
                max: Max::Bounded(1),
            },
            optionality: Optionality::O,
            length: Some(Length {
                min: 0,
                max: Some(1),
            }),
            conformance_length: None,
            table: Some(Table {
                id: "0269",
                value_set: "http://terminology.hl7.org/ValueSet/v2-0269",
            }),
            standards_status: None,
        },
    ],
};
