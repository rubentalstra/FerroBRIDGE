...
# Provenance: the FHIR R4B examples pass list

`pass-list.txt` records which resources of HL7's `hl7.fhir.r4b.examples` package
(4.3.0) round-trip through the `fhir-types` model of FHIR R4B, one file name
per line, and the resource count on its `total` line. It holds no copy of any
input. The package is fetched at build time into
`tools/ferrobridge-testkit/vendor/hl7.fhir.r4b.examples/` by
...
`fhir_examples::conformance_the_fhir_r4b_examples_hold_their_pass_list` in
