...
below, vendored verbatim from the FHIR package registry. Owner decision
2026-09-05: the crate and its generator move here from the sibling terminology
server, which then consumes the crate from crates.io. On 2026-09-12 the move
had not started (#72 carries it); the `PROVENANCE.md` paths below exist once
it lands, and the guard skips them until then.
...
inherits `fhir-types`, whose latest release from the sibling is the floor: the
sibling keeps publishing until the first release from here, so the row below
follows its latest release and the first publish from here is the next patch
above it. Every other library crate holds its name with a 0.0.0 placeholder
(#107), a version outside the line.

| Item | Pin | Repeated in |
|---|---|---|
| `fhir-types` | 0.1.97 | `docs/architecture.md`, later the `version` of every published `crates/*` manifest |
